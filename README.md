...
- **Channels + triggers** — conversational channels cover WhatsApp (WAHA), Telegram, Slack, Discord, and Playground under Hub → Channels; event triggers cover Email, Webhook, Jira, GitHub, and GitLab under Hub → Triggers; scheduled and recurring automation lives in Flows; GitHub/GitLab repository connections live under Hub → Developer Tools; Jira/Password Vault/search credentials are managed under Hub → Tool APIs; auto-provisioned Whisper/Speaches/Kokoro/Ollama under Hub → Local Services.
...
- **Password Vault foundation** — Hub → Tool APIs supports a provider-neutral Password Vault integration with 1Password as the first provider; setup is UI-first through Hub pickers. Agents can attach the Password Vault skill, and Flows can resolve explicit vault references through visible Flow steps while persisting only redacted outputs.
- **Variable Reference panel everywhere** — every templated step-config field (skill prompt, conversation objective, agentic gate prompt, slash-command body, gate-fail notification, etc.) gets the live Variable Reference panel with previous-step outputs + per-trigger-kind deep payload paths (Jira `payload.issue.key`, GitHub/GitLab repository event fields such as `payload.object.title` and `payload.repository.full_name`, etc.). Drag-and-drop chips into any field.
- **Code Repository skill (GitHub/GitLab)** — capability-gated repository skill (read on by default, write off by default) with reusable encrypted Hub repository connections and shared repository trigger criteria for push, PR/MR, issue, comment/note, release/tag, and workflow/pipeline events. Same contract as `ticket_management` (Jira) — same `WRITE` badges in the agent UI, same tool-spec gating so the LLM never even sees disabled actions.
...
| Channels and triggers (WhatsApp / Telegram / Slack / Discord / Playground; Email / Webhook / Jira / GitHub / GitLab) | [§14](docs/documentation.md#14-scheduler--triggers), [§15](docs/documentation.md#15-channels) |
