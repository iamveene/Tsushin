...
- **Continuous Agents** — wake on event/schedule, queue-driven runs, required Purpose + action_kind contract, and a Watcher run history that surfaces every wake event and outcome.
...
v0.7.0 reshapes the Hub around four roles — **Channels** (conversational), **Triggers** (event sources), **Tool APIs** (programmatic credentials), and **Local Services** (auto-provisioned Whisper/Speaches/Kokoro/Ollama) — and adds Agent Teams, Continuous Agents, multi-index vector stores, and a self-hosted ASR engine. Watcher owns wake-event, continuous-agent, and team-run monitoring; Flows own scheduled or recurring execution.
...
- **Continuous Agents** — wake-mode selector in the Studio New Agent modal (`Conversational` vs `Continuous`), required Purpose + action_kind contract, queue-driven runs, structured 409 detail surfaces actionable delete prompts, Watcher run history with wake-event evidence.
