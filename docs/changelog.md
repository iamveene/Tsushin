...
- **New trigger kind `github_commits`.** Polls a repo branch (default 300s), advances a single `last_seen_sha` cursor (commits on a branch are linear — no per-item snapshot table needed), and fires one templated WhatsApp message per new commit (**no LLM per commit**). The first poll seeds the cursor silently (no history backfill). Reuses the existing Hub `GitHubIntegration` PAT — no new integration row.
- **Delivery reuses the platform notification path:** dispatches through `TriggerDispatchService` → a notification-only system-managed Flow (Source → Gate → Notification) whose Notification node renders `{{source.payload.message}}`. Binds a `default_agent_id` (required so dispatch fans out to the bound flow) and sends through that agent's tenant WhatsApp MCP — **zero tokens** per commit.
- **New backend:** `channels/github_commits/trigger.py` (pure diff/compose engine + poller), `api/routes_github_commits_triggers.py` (CRUD + `/poll-now` + `/test-connection`), model `GitHubCommitsChannelInstance` (Alembic `0103`), `github_commits` registered in `TriggerDispatchService` + `flow_binding_service` + the scheduler poll loop.
- **Tests:** `backend/tests/test_github_commits_trigger.py` — new-commit diff (oldest-first ordering, cursor stop, cursor-fell-off-page, empty page), first-poll-seeds-silently, message composition, dedupe keys (12 cases).
- **Provisioning is API/flow-driven** (no new wizard UI this cut): create via `POST /api/triggers/github-commits` (or directly in the DB) bound to the desired agent + `@recipient`; the notification-only Flow is auto-generated.

### Added — GitHub Projects (v2) board → WhatsApp polling trigger + Code Repository read actions (2026-06-25)

GitHub does **not** emit webhooks for **user-owned** Projects v2 boards (only org-owned), so the existing PR-webhook trigger cannot observe a personal board. This adds a **polling** trigger that watches a Projects v2 board over the GraphQL API and notifies WhatsApp on three board events — mirroring the Jira polling-trigger architecture. **No version bump.**

- **New trigger kind `github_projects`.** Polls the board on an interval (default 300s), diffs a per-item snapshot, and fires a deterministic, templated WhatsApp message (no LLM per event) for: **new card**, **card assigned**, and **card moved** (Status single-select change, carrying from→to + when). The first poll seeds the snapshot silently (no backfill spam). Reuses the existing Hub `GitHubIntegration` PAT (requires the `read:project` scope) — no new integration row.
- **Delivery reuses the platform notification path:** the trigger dispatches through `TriggerDispatchService` → a notification-only system-managed Flow (Source → Gate → Notification) whose Notification node renders `{{source.payload.message}}`. New additive `notification_only` / `notification_template` options on `ensure_system_managed_flow_for_trigger` omit the conversation node so **zero tokens** are spent per board change. The trigger still binds a `default_agent_id` (required so dispatch fans out to the bound flow) and sends through that agent's tenant WhatsApp MCP.
- **Code Repository skill — 5 new read actions (default-ON):** `list_branches`, `list_commits` (REST) and `list_projects`, `read_project`, `list_project_items` (GraphQL), so an agent can answer "list branches / recent commits on <repo>" and "what's on my board". Write actions remain gated off; the new reads are GitHub-only.
- **New backend:** `hub/github/github_projects_service.py` (GraphQL client with `read:project` scope detection), `channels/github_projects/trigger.py` (pure diff engine + poller), `api/routes_github_projects_triggers.py` (CRUD + `/poll-now` + `/test-connection`), models `GitHubProjectsChannelInstance` + `GitHubProjectsItemState` (Alembic `0102`), `github_projects` registered in `TriggerDispatchService`, the scheduler poll loop, and the channel catalog. **Frontend:** a `github_projects` kind in the trigger wizard + a dedicated detail page + the triggers index listing.
- **Tests:** `backend/tests/test_github_projects_trigger.py` — diff engine (added/assigned/moved, dedupe keys, first-poll-seeds-silently, multi-assignee). Frontend `typecheck` + `lint` clean.
- **Limitation:** "who moved a card" is generally unavailable for Status changes on a user-owned board (the Projects v2 GraphQL node exposes no actor); messages carry reliable *what* (from→to) + *when*. Full who/what/when would require migrating the board to an Organization (org `projects_v2_item` webhooks include `sender`).

### Fixed — Recorder BUG-788: relay wedges under a slow/blocked client (2026-05-28)

Rebuilding the canonical Correios → @Vini flow **entirely through the recorder UI** surfaced a relay hang: while reading the streamed canvas, the recorder could permanently stop streaming frames AND silently stop processing every click/fill, while the WebSocket still showed "connected". Backend `event_count` froze, `ping` got no `pong`, and no inbound frames arrived — but no exception/close fired.

- **Root cause** (`backend/browser_recorder/cdp_relay.py`): the relay (a) sent each screencast frame to the client and only THEN `await`ed `Page.screencastFrameAck`, so a slow client backpressured the send and stalled Chromium's frame production; and (b) spawned an unbounded `websocket.send_json` task per frame with no single-writer discipline, so frame sends and the receive loop's event echoes raced on one Starlette WebSocket (concurrent `send_json` corrupts/wedges the connection). The receive loop also `await`ed its event echoes, so under backpressure it blocked and stopped dispatching input.
- **Fix:** every screencast frame is **ACKed to Chromium first** (never gated on client drain); all client-bound messages go through a **single writer task** (the only caller of `websocket.send_json`); **frames are lossy** (latest-frame-only, intermediates dropped); and the receive loop only **enqueues** (never blocks on a client send), so input is always dispatched regardless of client speed. Affects real users on slow/long-RTT links, backgrounded tabs (throttled rAF), or any heavy main-thread work in the tab.
- **Tests:** `backend/tests/test_recorder_relay_backpressure.py` (2 new) — input is still dispatched and frames still ACKed when the client's `send_json` hangs forever. Verified E2E through Caddy: after spamming `canvas.toDataURL()` (which previously wedged the relay), a subsequent canvas click still recorded (`click → input#objeto`). Full recorder suite: 58 pass.
...
- **Trailing `normalize_tracking` step (exposes a variable — no notification):** the recorder appends a `normalize_tracking` `data_transform` that surfaces the parser output as the reusable flow variable `{{normalize_tracking.data_preview.*}}` (`latest_status`, `latest_at`, `latest_location`, `event_count`, `latest_event_key`, `tracking_code`). The `flow_group` shape gained `trailing_nodes`. Sending is intentionally **not** wired by the recorder — see the "Changed" entry below.
...
- **Tests:** 5 new compiler unit tests in `backend/tests/test_browser_recorder_compiler.py`. Verified end-to-end through the UI: flow #277 (`UI Recorder V8b`) run #50173 delivered the exact canonical message to the local @Vini tester (`+5527999616279`), and on prod (flow #50, tenant `…91be5e`) the same message was delivered to @Vini's WhatsApp.

### Changed — Recorder/notification decoupling + captcha guidance (2026-05-28)

The recorder no longer owns "send a notification" — that duplicated the first-class **Notification** flow step. The recorder now *produces and exposes the data*; sending is a separate step the user adds. Cleaner separation, and the same `normalize_tracking` variable can feed any channel (WhatsApp, Telegram, Slack, webhook, …).

- **Removed the "Notify on finish" field** from the recorder dialog and the `notify_recipient` param from `POST /api/recorder/sessions/{id}/compile` (`frontend/app/flows/recorder/RecorderDialog.tsx`, `lib/client.ts`, `backend/api/routes_recorder.py`). A "📋 Capture timeline" recording now compiles to `browser_group` + `normalize_tracking` only (`backend/browser_recorder/event_compiler.py`).
- **To send the result**, add a **Notification** step after the recording and reference `{{normalize_tracking.data_preview.*}}` — e.g. `Status: {{normalize_tracking.data_preview.latest_status}}`. The dialog now shows an inline hint naming the available fields. Verified E2E (local flow #280 run #50176): recorder → `normalize_tracking` → user-added Notification step delivered the byte-exact canonical message (11 events, real status) to the @Vini tester.
- **Recorder guidance:** added a persistent "How to record" step strip (①–④) that makes explicit *when* to click **▣ Mark captcha** (when the page shows a captcha) and what to drag over (the captcha image), through to **📋 Capture timeline** and Save.

### Fixed — Browser Recorder: remaining deferred bugs closed (2026-05-28, follow-up)

Cleared every deferred recorder gap from the same investigation so a UI-only recording runs clean end to end:

- **solve_captcha false-negative → run reports clean `completed`** (`backend/browser_recorder/event_compiler.py`). The combined `solve_captcha` now receives a `success_selector` equal to the timeline root (`#tabs-rastreamento`) when a timeline capture follows it. Previously it OCR'd + submitted correctly but couldn't *confirm* the results loaded, so it reported "CAPTCHA was not solved" and the run ended `completed_with_errors` despite delivering the right message. The root only renders after a successful search (a failed captcha makes the `wait_for` for it time out), so it's a sound success barrier.
- **BUG-781 — session cold-start UX** (`frontend/app/flows/recorder/RecorderDialog.tsx`). Spawning the inner Chromium can take ~60–90s; the modal now shows an optimistic "Starting session… Ns" spinner with a live timer and disables Start while pending, instead of sitting on a blank "idle" canvas that looked broken.
- **BUG-782 — orphaned sessions no longer lock the per-tenant cap** (`backend/browser_recorder/session_manager.py`). `create()` now reaps sessions whose WebSocket relay has detached and which are idle past a 90s grace (tab closed / navigated away without Discard) before enforcing the max-2 cap — so an abandoned recording no longer 409s new Starts for the full 30-min TTL. Actively-relayed sessions are never reaped.
- **BUG-778 — automation typing amplification hardened** (`frontend/app/flows/recorder/StreamCanvas.tsx`). `handleKeyDown` now drops OS auto-repeat (`e.repeat`) and duplicate dispatches of the same physical event (identical `key`+`timeStamp`), which never eats genuinely-repeated characters (distinct timeStamps). The canvas **paste** path remains the recommended one-envelope route for bulk text under automation.
- **BUG-784 — recorded step thumbnails were broken `<img>`s** (`frontend/components/flows/BrowserGroupStep.tsx`). The `Thumb` "already a URL" check treated any `src` starting with `/` as a root-relative path — but JPEG frames are raw base64 that almost always begin with `/9j/`, so the browser fetched `/9j/…` → 404 → broken image (alt text shown) on every recorded flow's step screenshots. Now base64 is wrapped as a `data:image/jpeg;base64,` URI unless the value is genuinely a `data:`/`http(s)`/`blob:` URL or a path with a real file extension.
- **Tests:** +4 unit tests (compiler `success_selector` wiring ×2, session orphan-reap, no-timeline guard).
...
- **Step types removed:** `record_store`, `financial_record_store`, `financial_bill_store` (plus PascalCase legacy aliases `FinancialRecordStore` / `FinancialBillStore`). The matching `StepType` enum entries are gone from `backend/schemas.py`; the handler classes `FinancialRecordStoreStepHandler` and `FinancialBillStoreStepHandler` are gone from `backend/flows/flow_engine.py`; the frontend step palette and `FinancialRecordStoreConfigPanel` are gone from `frontend/app/flows/page.tsx`.
- **`data_transform` made generic again:** the `financial_parser` transform mode and the hardcoded `_parse_consigaz_bill` / `_parse_medsenior_bill` normalizers are gone. The node now supports only `json_path`, `extract_fields`, `parse_table`, `normalize_record`, `filter_records`, and `record_mapping`. The `emit_raw_bill_handle` / `emit_financial_record_handle` flags were also dropped.
- **Notification handler simplified:** `_enrich_delivery_only_secrets` (which existed solely to render `linha_digitavel` / `barcode_preview` for boleto notification templates) was removed. The generic `notification_state` template-switching feature stayed in.
- **`FinancialAutomationService` deleted:** the 657-line service with hardcoded Moderna/Consigaz/Medsenior providers, the BRL parser, the redaction whitelist for `_SENSITIVE_PAYLOAD_KEYS`, and the utility-bill `notification_state` classifier are all gone (`backend/services/financial_automation_service.py`). The `_classify_utility_bill_state` helper and its 7 boleto-specific states (`new_boleto`, `barcode_changed`, `pending_no_barcode`, `no_pending_bills`, `paid`, `unchanged`, `error`) no longer exist.
- **Models + tables dropped:** the `FinancialUtilityBill` and `FinancialAutomationRecord` SQLAlchemy classes were removed from `backend/models.py`. Alembic migration `0101_drop_financial_records.py` drops the `financial_utility_bill` and `financial_automation_record` tables. The originating migrations `0087` and `0088` are kept in the history.
- **Flow templates removed:** the dynamically generated `Finan | …` flow templates (built from `.private/finan_profiles.json`) are gone. `flow_template_seeding.py` is back to its 7 generic templates only: Daily Email Digest, Weekly Calendar Summary, Summarize on Demand, Proactive Watcher, New-Contact Welcome, Zero-Cost Inbox Monitor, Smart Email Filter.
- **Operator artifacts cleaned:** `.private/finan_profiles.json`, `.private/finan_playbooks/` (7 JSONs), `.private/financial-automation-migration/`, and `backend/data/financial-automation-browser/` were deleted from disk.
- **Existing flows audited and deleted:** the 6 `Finan | …` flow definitions (171, 172, 174, 177, 186, 187), the 2 QA fixtures of these nodes (243, 262), and their associated `flow_node` / `flow_run` / `flow_node_run` / `flow_trigger_binding` rows were removed by a one-off SQL cleanup. The non-finan Correios tracking flow (271) had inherited `record_kind` and `emit_financial_record_handle` keys via schema bleed — those keys were stripped from its `data_transform` node config and the flow continues to function unchanged on the generic `json_path` transform mode.
- **Tests:** `backend/tests/test_financial_record_dedupe.py` and `backend/tests/test_record_store_field_fallback.py` were deleted; `backend/tests/test_password_vault_contract.py` lost 17 tests that exercised the removed handlers/service and gained a stronger `test_legacy_financial_utility_automation_step_is_removed` guardrail asserting the new removals stay deleted.
- **Docs:** `README.md`, `docs/user-guide.md`, and `docs/documentation.md` were trimmed of finan-specific sections (UI-first financial setup, financial templates section, notification-state classifier doc, Financial Record Store / Utility Bill Store table rows, etc.).

Operators with existing tenants on this branch must accept that `financial_utility_bill` and `financial_automation_record` data is **dropped**, not migrated — the cleanup was an explicit goal, not a regression.

### Fixed — Browser Recorder full end-to-end overhaul: 11 bugs closed (2026-05-27)

A clean-slate UI walkthrough of the canonical Correios → @Vini WhatsApp flow on both `https://tsushin.archsec.io` (prod) and `https://localhost` (with the local @Vini tester WhatsApp instance on port 8082) surfaced 11 independent bugs that together prevented a freshly-recorded Browser Automation flow from replaying correctly. **All 11 are fixed and shipped to main** (PRs [#214](https://github.com/iamveene/Tsushin/pull/214) + [#216](https://github.com/iamveene/Tsushin/pull/216) + [#218](https://github.com/iamveene/Tsushin/pull/218) + [#220](https://github.com/iamveene/Tsushin/pull/220)). The canonical local E2E smoke test (`backend/scripts/recorder_e2e_correios_to_vini.py`) now reports `OVERALL: PASS` and the structured tracking message reaches Vini's local tester. Run #50171 of the rebuilt flow #271 confirmed `Status: Objeto entregue ao destinatário. … Events: 11 …` delivered to `mcp-tsushin-tester-0fe9a9f6-6:8080/api`.

Counts: **41 compiler tests + 6 session_manager tests passing** (10 new regressions added: 3 for BUG-768, 2 for BUG-771, 1 for BUG-773, 2 for BUG-774, 1 for BUG-776, plus an updated canonical captcha-chain test that now asserts wait_for is auto-inserted between solve_captcha and extract).

| Bug | Severity | File(s) | Summary |
|---|---|---|---|
| **BUG-767** | High | [`frontend/app/flows/recorder/StreamCanvas.tsx`](../frontend/app/flows/recorder/StreamCanvas.tsx) | `pointerDown` didn't grab DOM focus on the canvas, so the user's first keystroke went to whatever element previously had focus. Confirmed user-side: *"nor typing or clipboard paste worked on the text field to type in the tracking number"*. Fix: `canvasRef.current?.focus({ preventScroll: true })` inside `handlePointerDown`. |
| **BUG-768** | Critical | [`backend/browser_recorder/cdp_relay.py`](../backend/browser_recorder/cdp_relay.py) + [`session_manager.py`](../backend/browser_recorder/session_manager.py) + [`event_compiler.py`](../backend/browser_recorder/event_compiler.py) | The `input.text` handler shipped FILL events with `selector=None` because the frontend doesn't know the inner-Chromium focused element; the compiler then fell back to `body`, breaking solve_captcha (`value_target = body` → *"Element not found or not fillable: body"*) and silently dropping every tracking-code fill. Fix: added `__tsushinFocusedSelector` page shim, call it from `_handle_client_message` when the frontend omits the selector, and harden the compiler so root selectors (`body`/`html`/`*`/`document`) are scrubbed and emit `_needs_selector` instead of silently shipping broken rows. |
| **BUG-769** | High | [`frontend/app/flows/recorder/StepLedger.tsx`](../frontend/app/flows/recorder/StepLedger.tsx) | Each typed character was its own `FILL` row (13 rows for `AD468811215BR`). Added `coalesceForDisplay` so the ledger collapses consecutive fills on the same selector — what you see now matches what the compiler emits. |
| **BUG-770** | Medium | [`frontend/app/flows/recorder/RecorderDialog.tsx`](../frontend/app/flows/recorder/RecorderDialog.tsx) | `Capture output` used a native `window.prompt` that blocked the Chrome tab (CDP timeouts in automation), used unstyled chrome, and still wired the marker even when the user pressed Cancel. Replaced with an inline `Modal` that defaults the variable name to `delivery_status`, shows the `{{previous_step.<name>}}` template hint live, and **genuinely cancels** on dismiss. |
| **BUG-771** | Medium | `event_compiler.py` | Two consecutive identical clicks compiled to two distinct flow steps. Added `_dedupe_consecutive_clicks` mirroring the existing focus-click-then-fill dedupe. |
| **BUG-772** | High | `StreamCanvas.tsx` | `handleKeyDown` fired BOTH `onKey` (CDP dispatchKeyEvent inserts the char too) AND `onText` (CDP insertText) for printable chars — doubling every keystroke. Fix: printable single chars take the text-insert path exclusively; non-printables (Tab/Enter/Backspace/etc.) keep the key-event path. |
| **BUG-773** | High | `event_compiler.py` (`_combine_captcha_chain`) | The captcha combiner picked the *first* click after the captcha marker as form submit — almost always the focus-click into the captcha input, not the real Consultar button. Added `_looks_like_submit` (matches `button`/`submit`/`pesquisar`/`consultar`/`buscar`/`search`) + a `_is_text_input` skip-list so the resolver picks `button[name="b-pesquisar"]` correctly. |
| **BUG-774** | High | `event_compiler.py` (`_combine_captcha_chain`) | `success_selector` was unconditionally set to whatever the recorder marked as extract — on captcha-gated sites the user can only mark a noise region (carousel) because results don't render at record time, so at runtime the noise selector matched immediately and the captcha skill exited before the page settled. Now only content-region selectors (`ship-step` / `result` / `tracking` / `rastreamento` / `evento` / `timeline` / `status-list` / `history`) become `success_selector`. |
| **BUG-775** | High | [`backend/flows/flow_engine.py`](../backend/flows/flow_engine.py) (`_resolve_mcp_url`) | The MCP resolver only accepted `instance_type='agent'` and fell through to `http://127.0.0.1:8080/api` when a tenant only had a `tester` instance — which is the default local-dev shape (e.g. Vini's local tester runs on port 8082). Notifications then silently 404'd. Now the resolver falls back to `tester` instances per-tenant before defaulting. |
| **BUG-776** | Medium | `event_compiler.py` (`_combine_captcha_chain`) | When the recorder didn't capture an explicit `wait_for` between submit and extract, the runtime's extract raced the page reload and pulled empty/wrong content. Compiler now auto-emits a `wait_for` targeting the extract's selector (when content-region) — mirrors the canonical `wait_tracking_result` shape. |
| **BUG-777** | Medium | [`frontend/app/flows/page.tsx`](../frontend/app/flows/page.tsx) | Per user-guide §9, every flow should observe its outcome via a trailing notification, but the wizard defaulted child browser steps to `on_failure='stop'` so any single bad step silently swallowed the alert. Now recorder-built browser children + new Notification steps default to `on_failure='continue'`. |
| **BUG-778** | Known-limitation, documented | `StreamCanvas.tsx` | OS-level keyboard simulation (Playwright `computer.type`, Claude-in-Chrome) often delivers each character as 2-3 keydown events within microseconds AND the inter-char gap can be just as small — so a time-based "drop duplicates within Xms" filter either keeps the noise (window too tight) or eats legitimate consecutive same-char typing like `"88"`/`"11"` in tracking codes (window too wide). Real users typing in a browser don't trigger the amplification — this is a test-tool artefact. Recommended workflow when driving the recorder from automation: paste the text via the canvas's `onPaste` handler (one envelope per paste, no per-keystroke noise) OR fix the fill value post-record in the wizard step editor. |

End-to-end proof:
- **Local stack** (Vini's local tester, port 8082, authenticated): Flow #271 `Correios | AD468811215BR | UI Recorder V7` built today via the recorder UI — Run #50171 delivered the structured message (`Status: Objeto entregue ao destinatário. / When: 22/05/2026 15:46 / Location: … VILA VELHA - ES / Events: 11 / Dedupe: 22-05-2026-15-46:…`) to `mcp-tsushin-tester-0fe9a9f6-6:8080/api`.
- **Prod** (`https://tsushin.archsec.io`): Flow #48 `UI Recorder | Correios + Notify Vini | 20260527T013047Z` built today via the recorder UI on prod — Run #2987 delivered the same clean format to `+5527999616279`.
...
- Added a new `browser_group` step type (backend [`StepType.BROWSER_GROUP`](../backend/schemas.py) + [`BrowserGroupStepHandler`](../backend/flows/flow_engine.py)) that is a pure UI marker — its handler is a no-op pass-through (returns `completed` in 0 ms). The children that follow it (`browser_automation` steps sharing a `group_recording_id`) run exactly as they always have; only the editor and watcher rendering change.
- Added a third compile shape to `POST /api/recorder/sessions/{id}/compile`: `flow_group = { group_node, child_nodes }` alongside the existing `config_json` and `flow_nodes`. Children are annotated with `group_recording_id`, `group_index`, `recorded_driver` (`"human" | "agent" | "mixed"`), `recorded_at`, and `screenshot_b64` so the card can render per-step thumbnails and a provenance badge without an extra fetch.
- Captured a JPEG screenshot at the moment of every recorded event by caching the latest CDP screencast frame on `RecordingSession.latest_frame_b64` ([`backend/browser_recorder/cdp_relay.py`](../backend/browser_recorder/cdp_relay.py)) and attaching it inside `RecordingSession.append_event`. The driver source (`RecordingDriver.HUMAN | .AGENT`) is mirrored onto each `RecordedEvent` so toggling agentic mode mid-session keeps per-step provenance.
- Added [`compile_events_into_group(events, *, recording_id)`](../backend/browser_recorder/event_compiler.py) that derives target host (from first navigate), driver label (homogeneous → human/agent, heterogeneous → mixed), recorded-at timestamp, and child screenshots (one per action event, with fill coalesce honoured), then wraps them as `{group_node, child_nodes}`. 6 new unit tests cover the parent-plus-children shape, per-event screenshot attachment, mixed driver collapse, fill-coalesce screenshot pick, and empty-events safety.
- Added the [`BrowserGroupStep`](../frontend/components/flows/BrowserGroupStep.tsx) collapsible card React component with `mode="edit"` (flow editor — shows recorded thumbnail per child + Ungroup affordance) and `mode="run"` (watcher run-detail — adds runtime thumbnails side-by-side with per-child status chips). Action chips colored per `tool_action` (cyan navigate, emerald fill, sky click, amber solve_captcha, fuchsia extract); driver badge colored emerald (human) / violet (agent) / amber (mixed).
- Wired the card into both `StepBuilder` and `EditableStepBuilder` in [`frontend/app/flows/page.tsx`](../frontend/app/flows/page.tsx) via a shared [`groupBrowserSteps`](../frontend/lib/browserGroupHelpers.ts) helper that folds either real `browser_group` parents + their `group_recording_id`-matched children, OR — for backwards compat — ≥2 consecutive `browser_automation` steps with no parent. Synthetic groups carry an amber "Auto-grouped — save to persist" hint so legacy flat flows render as one card today without any migration.
- Wired the same card into the run-detail modal (`ViewRunModal`); the modal now fetches the flow's `FlowNode` list once per run so per-step `config_json.screenshot_b64` joins with `FlowNodeRun.output_json.screenshot_paths` for the side-by-side recorded-vs-runtime view auditors asked for.
- Verified end-to-end on `https://localhost` against fixture flow #257 (parent `browser_group` + 2 children = navigate example.com + extract h1) and a legacy flat fixture #258. Group card collapsed/expanded, runtime thumbnails in the run-detail modal, agent/human badge, Ungroup affordance (deletes the parent step via the existing `deleteFlowStep` API and resynthesizes the children as a synthetic group), and 0 console errors throughout. The recorder /compile endpoint returns the new `flow_group` field correctly.
- Deferred to a follow-up: making the in-modal Save button in `RecorderDialog` POST the group + children to the flow via `createFlowStep` (today it still merges a single `config_json` into one `BrowserAutomationConfigPanel`). The synthetic auto-grouping covers the visible UX win without that refactor, but a full record→group insert path is the natural next step.
...
- Added a `ToolPalette` with **▣ Mark captcha** and **👁 Capture output** drag-overlay tools that translate directly into the existing `solve_captcha` and `extract` selector actions; `solve_captcha` rows auto-wire their `value_target` to the next captured fill row, matching the canonical Correios postal-tracking shape.
- Added a **🔑 Vault chip** on still-plaintext password rows in the recorder ledger that opens the existing `PasswordVaultReferencePicker`; on confirm the recorder swaps the field's value for the picked `op://`-style reference and emits a `browser_secret_references` row pointing at `selectors[N].value`. The compile call refuses to emit any plaintext password — defense in depth.
- Added an opt-in **Agentic mode** that lets a Browser-Use agent drive the same recording session from a free-form prompt (Claude Opus 4.5 planner + Haiku 4.5 per-step, two-LLM split). Disabled by default — install `browser-use` from `requirements-optional.txt` to enable. Agent credentials are resolved per-tenant via `ProviderInstance` (same path `AIClient` uses), preserving multi-tenant isolation; the route returns a specific 503 when the tenant hasn't configured an Anthropic provider. Post-run history is walked and translated into RecordedEvents so the compiler produces a real FlowNode from agentic recordings.
- Added per-tenant **concurrency cap** (2 active recordings/tenant, HTTP 409 over the limit) and a 60-second **TTL janitor** that reaps recordings idle past 30 minutes so a forgotten dialog can't leak a Chromium instance. Per-step `asyncio.wait_for(5s)` budgets on each Playwright closer keep `DELETE /api/recorder/sessions/{id}` returning in under 100ms even when the underlying browser is slow to close.
- Added `/api/recorder/*` REST endpoints (sessions, compile, agent start/pause, debug) plus a `/ws/recorder/{id}` cookie-authed WebSocket that mirrors the existing `watcher_activity_websocket` handshake (httpOnly cookie primary, first-message JWT fallback), with full tenant-hopping and revoked-user fail-closed checks from `shell_websocket`.
- Added a `GET /api/recorder/sessions/{id}/debug` introspection endpoint that returns live session state, the agent task's exception (if any), and the full RecordedEvent stream — so the UI and humans can see why an agentic recording stalled without spelunking server logs.
- Configured the recorder to navigate with `wait_until="domcontentloaded"` and a 45 s ceiling instead of the Playwright default `wait_until="load"`; real sites (Correios analytics, third-party banners) used to hang the initial goto for the full default timeout. Real Correios now loads in ~1.1 s.
- Added a `_dedupe_focus_click_then_fill` post-pass to the event compiler so the focus-click that precedes typing into an input collapses into the `fill` row alone (Playwright's `fill` handles focus). Eliminates dead-weight rows in compiled flows.
- Added 21 unit tests for the compiler (Correios-shaped event sequences, selector strategy ladder, password detection, captcha `value_target` wiring, `pvh_`/`op://` vault round-trips, click-before-fill dedupe, `tool_action` default).
- Made the recorder emit a top-level `"tool_action": "navigate"` on every compiled `config_json` — the manual config panel sets this via its dropdown default, and without it `BrowserAutomationStepHandler` fails the step at execution time with "Missing tool_action for browser automation step". Recorded flows now run cleanly against the runtime without any post-recording manual editing.
- Established a **project-wide convention**: every browser-automation flow ends with a Notification step (channel `whatsapp`, recipient `@Vini`) so the success/failure is observable. The recorder doesn't append the notification row automatically — the user-guide worked example walks through adding it. A new smoke-test script at [`backend/scripts/recorder_e2e_correios_to_vini.py`](../backend/scripts/recorder_e2e_correios_to_vini.py) drives the full loop (record → save → execute → notify Vini) and reports structured proof of the notification leg (resolved recipient, rendered message body, MCP URL) even when WhatsApp delivery is blocked on an unauthenticated tenant MCP.
- Verified end-to-end against the official **`rastreamento.correios.com.br/app/index.php`** Securimage-captcha site: a complete recording produces the canonical `selectors[]` array (fill `objeto` → `solve_captcha img#captcha_image` with `value_target` wired to `input[name="captcha"]` → fill captcha → click `button#b-pesquisar` → extract result panel `as=delivery_status`) — bit-for-bit compatible with what `BrowserAutomationStepHandler` already executes.

### Added — Recorder produces ONE FlowNode per browser action (2026-05-23)

Resolves the gap where the recorder packed every captured action into one `FlowNode.selectors[]` but `BrowserAutomationStepHandler` only executes the top-level `tool_action` (one action per FlowNode) — chains never replayed at runtime. Adds `compile_events_into_nodes(events) -> list[dict]` alongside the legacy `compile_events()`. Each emitted node has a single `tool_action` matching the captured action, with full canonical shape (one selector per node, shared `session_persistence=True` + `session_ttl_seconds=1800` so the Playwright context carries across nodes). The `/api/recorder/sessions/{id}/compile` route now returns BOTH shapes:
...
so the existing UI integration (BrowserAutomationConfigPanel single-step) keeps working while programmatic consumers (`backend/scripts/recorder_e2e_correios_to_vini.py` and friends) use the production-ready shape.

Captcha chains get an extra collapse pass — when the recorder captures `marker.captcha → click submit → marker.extract`, the multi-FlowNode compiler folds them into ONE canonical `solve_captcha` FlowNode with the `tool_arguments` dict shape (`selector`, `input_selector`, `submit_selector`, `success_selector`) the runtime's combined OCR+fill+submit+wait skill expects. Defaults `solver_provider: "gemini"` + `solver_timeout_seconds: 120` because Ollama cold-start typically exceeds the runtime's 60s httpx ReadTimeout default on tenants where the multimodal model isn't already loaded.

Also added diagnostic instrumentation to `BrowserAutomationSkill`'s captcha solver exception handler — previously formatted as `f"CAPTCHA solver failed: {exc}"`, which renders as a useless empty string when `str(exc)` is empty (e.g. `httpx.ReadTimeout('')`). Now logs the full traceback and includes the exception type name + `repr(exc)` fallback so solver failures are debuggable.

Validated end-to-end on prod with flow #43 ("Recorder Demo | Correios | AD468811215BR (canonical via compile_events_into_nodes)") — 6 steps generated by the recorder + 2 hand-added (data_transform + notification), 14-second execution, all 6/6 step runs completed, 10/10 hard validations PASS, canonical Correios tracking message delivered to `@Vini` on WhatsApp from a recorder-produced flow. 26/26 compiler unit tests green.
...
- Added Alembic backfills that attach existing generated GitHub/GitLab review teams to the creator's mapped reachable contact, then to an active tenant owner's mapped reachable contact for historical teams created by inactive QA users.
...
- **Backend** (centralized skill registry): introduced `backend/constants/skill_metadata.py` as the single source of truth for skill display metadata and the save-endpoint validator allow-list. `get_valid_skill_types()` now derives from the live `SkillManager.get_registered_skill_types()` plus legacy aliases (`calendar` / `asana` / `email`). The three formerly-duplicate `SKILL_METADATA` dicts in `backend/api/routes_agent_builder.py`, `backend/api/v1/routes_studio.py`, and `backend/api/routes_agents_protected.py` now import from this module. Newly-registered skills automatically pass save validation — preventing the `password_vault` / `okg_term_memory` / `ticket_management` / `code_repository` / `agent_communication` drift that had silently shipped with v0.7.0. Includes `backend/tests/test_skill_registry_integrity.py` as a guardrail against future drift.
- **Backend** (`backend/agent/skills/audio_transcript.py`): documented that the cloud-only `model` enum is ignored when `asr_mode='instance'` (the local container's `default_model` is the runtime source of truth).
- **Frontend** (`frontend/components/audio-wizard/AudioProviderFields.tsx`): the ASR instance list now refetches when `asr_mode` changes and listens for a new `tsushin:asr-instance-changed` event emitted by the provider wizard on successful create — eliminating the stale-instances state that required closing/reopening the modal to pick up a newly-provisioned instance. The model field renders mode-aware: an editable cloud-model dropdown in `openai` mode, a read-only `vendor · default_model` label in `instance` mode (matching the user spec "for local instances with single model, read only, no selection at all"). The cloud-model catalog moved to a data-driven const so adding a future cloud model (e.g. `gpt-4o-transcribe`) is a one-line change.
- **Frontend** (`frontend/components/provider-wizard/steps/StepProgress.tsx`): emits `tsushin:asr-instance-changed` after a successful `createASRInstance` so open agent-config panels can refresh without coordination.
- Verified end-to-end: 5/5 `test_skill_registry_integrity.py` tests pass on both local and production containers; production backend reports 26/26 `SKILL_METADATA` keys covered by `get_valid_skill_types()`, including `password_vault` and `okg_term_memory`. Browser QA on `https://localhost` confirmed mode-switching behaves correctly and that saving an agent with `password_vault` enabled returns 200 with no `invalid skill_type` toast.
...
- Hid the aborted PMVV profile and the pending Husky profile from the public financial template picker, so the catalog only advertises workflows that are part of the accepted migration set.
- Corrected the accepted financial Flow notification nodes to target `@Vini`, resolving to WhatsApp `+5527999616279`, and verified the wrong local test number is no longer present in the Flow DB/config surface.
- Simplified the financial template wizard by collapsing technical fields under Advanced options and associating wizard/editor labels with real inputs for Flow name, recipient, Password Vault picker fields, and Browser Automation URL/action controls.
- Fixed Utility Bill Store normalization so `automation_key` is mirrored from config/legacy `automation_id` before upsert, keeping utility-bill dedupe stable for UI-created financial templates.
- Revalidated the Consigaz template from the UI after the gap review: From Template -> preview 16 visible nodes -> create real Flow -> open editor -> customize browser URL/action controls -> add Skill/Summarization/Notification options -> delete the temporary QA Flow.
...
- Added the first accepted UI-first financial workflow template catalog for Cond. São Blas, Consigaz, Medsenior/Samedil, Cypreste/Superlógica, and EDP. DETRAN-ES and B3 were intentionally removed from the active migration scope for this round; PMVV is kept as a private aborted profile until its CAPTCHA boundary has an accepted solver/manual handoff; Husky remains a private pending profile until it passes the same UI recreation, run, dedupe, and notification validation. Public templates import the corresponding Finan playbook actions into visible nodes rather than `financial_utility_automation`, including wait-for-selector, wait-for-URL, dismiss-modal, execute-script, and explicit provider extraction steps where present.
