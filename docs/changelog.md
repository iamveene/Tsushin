...
- **Graph View playground glow — BUG-597.** Pipeline verified correct end-to-end: backend emits `emit_agent_processing_async(channel="playground")` at 5 sites in `playground_service.py`; `useWatcherActivity` hook captures the channel; `GraphCanvas` applies `edge-active-cyan`; CSS `.edge-active-cyan` animates via `edge-glow-pulse-cyan`. No code change required — the original audit's observation is a derivative of the Cluster B backend instability (WebSocket drop during pool exhaustion). Re-verified via Playwright after the rebuild.
