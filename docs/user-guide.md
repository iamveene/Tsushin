...
After first login, a 16-step onboarding tour auto-opens. The first step welcomes you, steps 2–5 are a "What's New" showcase covering the expanded AI provider catalog (Vertex AI, Grok, Groq, ElevenLabs), Slack/Discord channels + Webhook Triggers, Custom Skills & MCP Servers, and A2A agent-to-agent permissioning + external vector stores. Steps 6–11 walk through platform areas (Watcher → Studio → Hub → Channels → Flows → Playground). Steps 12–15 cover optional voice setup, Playground Mini, Sentinel block mode, and v0.7.0 Triggers & Continuous Agents. Step 16 is the "You're All Set" finale. Each showcase step includes a one-click deep-link to the relevant Hub tab or sub-page. You can minimize the tour at any time with the chevron icon — a "Continue tour" pill survives a full page reload — or dismiss it permanently with ×.
