...
3. The capability matrix is split into **read** (default ON) and **write** (default OFF) groups:
   - **Read:** `search_repos`, `list_pull_requests`, `read_pull_request`, `list_issues`, `read_issue`, and (GitHub only) `list_branches`, `list_commits`, `list_projects`, `read_project`, `list_project_items`. The last three read GitHub Projects v2 boards over GraphQL, so the connection's token needs the `read:project` scope.
...
- **GitHub Projects** -- a GitHub Projects v2 board (the new Projects), **polled** on an interval. Because GitHub does **not** send webhooks for user-owned boards, Tsushin watches the board over the GraphQL API and diffs its state, sending a WhatsApp message whenever a card is **added**, **assigned**, or **moved between columns** (the board's Status field). Reuses a Hub > Developer Tools GitHub connection whose token has the `read:project` scope; you provide the board owner and number (from the board URL `…/users/<owner>/projects/<number>`), a poll interval, the delivery agent, and the recipient (e.g. `@Vini`). Unlike the other kinds, this trigger sends a fixed templated message (no AI is invoked per event) and has no event filters yet — it notifies on every card change. Use **Test connection** in the wizard to confirm the PAT can read the board before saving. Its auto-flow is notification-only (Source → Gate → Notification — no Conversation step).
- **GitHub commits** -- watches a repository **branch** for new commits, **polled** on an interval. GitHub *can* send push webhooks, but they cannot reach a Tsushin behind an IP allowlist, so Tsushin checks the branch's commits over the GitHub API and notifies on each **new commit** (`🔨 New commit on owner/repo@branch: "subject" — author (sha)`). Reuses a Hub > Developer Tools GitHub connection; you provide the repository, the branch to watch (e.g. `develop`), a poll interval, the delivery agent, and the recipient (e.g. `@Playground`). Like the Projects trigger it sends a fixed templated message (no AI per commit) and its auto-flow is notification-only (Source → Gate → Notification). The first check after creation establishes a baseline silently — you start getting messages from the *next* commit onward. (Provisioned via the API/flow config in this release; no dedicated wizard step yet.)
