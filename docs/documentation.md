...
| `BACKEND_INTERNAL_URL` | `http://${TSN_STACK_NAME:-tsushin}-backend:8081` | Runtime backend origin used by Next.js rewrites and the auth proxy inside Docker. Keep stack-scoped on the shared `tsushin-network`. | — | `docker-compose.yml:271`, `frontend/next.config.mjs:24`, `frontend/app/api/auth/[...path]/route.ts:21` |
