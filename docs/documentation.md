...
* **Pre-existing follow-up surfaced by QA** — when `validate_flow_structure` fails inside `run_flow`, the engine creates a *new* failed FlowRun rather than mutating the API-created pending one. The result is that `POST /api/flows/{id}/execute` returns 202 and the original FlowRun stays `pending` forever even though a separate failed FlowRun is recorded. Affects all validation failures, not just `triggered`. Tracked for a future polish wave.
