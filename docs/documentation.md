...
**Tsushin** (通信 — Japanese for "Communication") is a multi-tenant, self-hostable agentic messaging platform that unifies AI agent orchestration, conversational channels (WhatsApp, Telegram, Slack, Discord, Playground), event triggers (Email/Gmail, Webhook, Jira, GitHub, GitLab), Tool-API credentials and repository integrations (Jira, GitHub, GitLab, 1Password Password Vault), local services (Whisper/Speaches, Kokoro, Ollama), Agent Teams (line + mesh), Continuous Agents, multi-layer semantic memory, workflow automation (Flows), AI-powered security (Sentinel), and observability.
...
**Tsushin** (通信 — Japanese for "Communication") is a multi-tenant, agentic messaging framework that unifies AI agent orchestration, conversational channels (WhatsApp, Telegram, Slack, Discord, Playground), event triggers (Email/Gmail, HTTP Webhook, Jira, GitHub, GitLab), Tool-API credentials and repository integrations (Jira, GitHub, GitLab, 1Password Password Vault), local-service containers (Whisper/Speaches, Kokoro, Ollama), Agent Teams, Continuous Agents, a multi-layer semantic memory system, workflow automation ("Flows"), AI-powered security (Sentinel), and observability under a single self-hostable platform.
...
**Multi-stack note:** Proxy and frontend upstreams must target stack-scoped container names (for example `tsushin-frontend:3030`, `tsushin-backend:8081`) rather than generic Docker aliases like `frontend` / `backend`. On the shared `tsushin-network`, generic aliases can resolve to another running Tsushin stack. BUG-723 closed the remaining HTTP-mode gap by making `BACKEND_INTERNAL_URL`, `INTERNAL_API_URL`, the frontend build args used by Next fallback rewrites, and `proxy/Caddyfile` all default to `${TSN_STACK_NAME}-backend` / `${TSN_STACK_NAME}-frontend`.

**Browser → backend request path (v0.6.1, updated for BUG-723):** the Next.js frontend proxies general `/api/*` and `/ws/*` traffic to the backend via fallback `rewrites()` declared at `frontend/next.config.mjs`, after first-party route handlers have had a chance to run. Standalone rewrites are evaluated during the frontend image build, so compose passes `BACKEND_INTERNAL_URL` as a build arg and as a runtime env var; it defaults to `http://${TSN_STACK_NAME:-tsushin}-backend:8081`, the stack-scoped compose service name. Auth endpoints are the deliberate exception: `/api/auth/*` is handled by `frontend/app/api/auth/[...path]/route.ts`, which forwards the request to the same stack-scoped backend origin at runtime and explicitly preserves `Set-Cookie` response headers. This keeps browser requests same-origin with the frontend, while ensuring the httpOnly `tsushin_session` cookie is actually stored after login/signup/setup/SSO/logout in both HTTP and TLS installs. The auth proxy normalizes `x-forwarded-proto` from `TSN_SSL_MODE`, so HTTP installs ask the backend for non-`Secure` cookies and HTTPS installs keep `Secure` cookies. Replaces the v0.6.0 pattern of baking an absolute `NEXT_PUBLIC_API_URL` into the build, which dropped the cookie on cross-origin access.
...
Frontend (Next.js)  <--REST/WebSocket-->  Backend (FastAPI + PG 16)  <-->  RBAC Layer
              ...
              TRIGGERS (Email, Webhook, Jira, GitHub, GitLab)  |  TOOL APIs / REPOSITORY INTEGRATIONS (Jira, GitHub, GitLab, 1Password)
...
* **Sentinel detection defaults** — `continuous_agent_action_approval` is registered as a first-class detection type and exposed through `/api/sentinel/detection-types`. Alembic `0055` widens Sentinel detection-type log/cache columns to 64 characters so this long detection id can be cached and audited without truncation.
...
### 2.4.5 v0.7.0 Agent Teams Phase 5 CRUD API layer

Phase 5 is scoped to the backend HTTP API layer for Agent Teams. The session-authenticated Studio surface is `/api/teams`; the public API client mirror is `/api/v1/teams`. Both surfaces expose tenant-scoped team CRUD, member management, run listing/detail, run cancellation, and manual-run request shapes so the frontend can be added later without inventing a second contract.
...
Each binding stores a canonical config in `agent_team_trigger.config_json`: `trigger_instance_id`, `event_types`, `filters`, and `is_enabled`. The API validates that the trigger instance is active, belongs to the same tenant, and is one of Webhook, GitHub, GitLab, Jira, or Gmail.
...
### 2.4.8 v0.7.0 Agent Teams Phase 8 Team Wizard

Phase 8 turns the `/studio/teams?new=1` hook into a full creation workflow. A global Team Wizard provider owns the draft, persists meaningful in-progress fields in local storage under `tsushin.team_wizard.draft`, and opens from both `/studio/teams?new=1` and the preserved `/agents/teams?new=1` redirect path.

The wizard includes Custom/Template, Basics, Topology, Members, Tools, Triggers, Review, and Create steps. Templates are local presets only: they prefill the draft but do not create persistent seeded template records or clone from a template endpoint. Members are selected from active agents that are not already assigned to another team; line-topology ordering can be adjusted before creation. The tool pool binds enabled sandboxed tools through `tools.sandboxed_tool_ids`.

Trigger bindings in the wizard attach existing active Webhook, GitHub, GitLab, Jira, or Gmail trigger instances through `/api/teams/{team_id}/triggers`. Each binding sends `trigger_kind`, `trigger_instance_id`, optional `event_types`, JSON object filters, and `is_enabled`.

Creation posts to `/api/teams`, then posts any trigger bindings, clears the persisted draft, and redirects to `/studio/teams/{id}`. Phase 8 includes a minimal team detail landing for created teams with goal, member, tool, trigger, and last-run summaries; visual builder/canvas editing stays deferred to Phase 9.
...
* **Fail-closed outbound scopes** — send/reply now require a Gmail send-compatible scope (`gmail.send`, `gmail.compose`, `gmail.modify`, or `mail.google.com/`), while draft creation requires a draft-compatible scope (`gmail.compose`, `gmail.modify`, or `mail.google.com/`). Older read-only OAuth tokens stay readable but cannot send until the tenant reauthorizes.
* **Hub/API reauth contract** — Gmail reauthorization surfaces request `include_send_scope=true`, which now adds both `gmail.send` and `gmail.compose`, so the upgrade path for full outbound send + draft support is explicit in both API and UI flows.
* **User-facing capability copy** — the Gmail setup wizard and `/email info` output now distinguish between "send/reply only" and full "send + draft" readiness instead of assuming `gmail.send` covers every outbound action.
* **Regression coverage** — `backend/tests/test_gmail_send_phase3_checkpoint.py` covers send, reply, draft, and scope-gating behavior. `backend/tests/test_gmail_send_phase3_live_gate.py` is an opt-in root-only live proof that stays skipped unless `TSN_RUN_GMAIL_PHASE3_LIVE_GATE=1` is set; once enabled, it fails if the fixture lacks a draft-compatible Gmail scope. The fixture was reauthorized with `gmail.compose` for `mv@archsec.io` during the Phase 3 exit gate, and the live proof now covers direct send, direct reply, `GmailSkill` send, and draft creation. The optional agent-chat/API proof still requires `TSN_RUN_GMAIL_AGENT_CHAT_LIVE_GATE=1` plus API endpoint/token/agent env vars.
...
* **Auth and permissions** — continuous-agent list/detail uses session cookie, UI Bearer JWT, OAuth2 API-client Bearer JWT, or `X-API-Key` via the shared auth stack. `/api/continuous-agents*` requires `agents.read`; `/api/continuous-runs*` and `/api/wake-events*` require `watcher.read`. The standard `api_readonly`, `api_member`, `api_admin`, and `api_owner` client roles include `watcher.read` for these read surfaces. Missing tenant context fails closed with `403`; cross-tenant detail access returns `403`.
...
* **Multilingual follow-up coverage (BUG-706)** — `FOLLOWUP_PATTERNS` covers English interrogatives (`what`, `which`, `who`, `whom`, `whose`, `where`, `when`, `why`, `how`), pronoun phrasings (`that one`, `the first/last/previous one`, `the one (that|which|with)`), Portuguese/Spanish interrogatives (`o que`, `quem`, `onde`, `quando`, `por que`, `como`, `qual`), and PT demonstratives (`esse`, `essa`, `aquele`, etc.). Fresh-fetch phrases (`latest`, `recent`, `novos`, `buscar de novo`) still win and force a new tool call.
...
* **Breadth handoff** — the dispatch foundation is now consumed by Jira, GitHub, GitLab, and Webhook criteria. `continuous_task` queue dispatch remains reserved until intentionally implemented.

### 2.11 v0.7.0 Track B Trigger Breadth

Track B breadth turns the dispatch foundation into user-facing trigger adapter rows and Hub UI. It established the Jira/GitHub/Webhook CRUD contracts, criteria surfaces, and normalization tests, then the v0.7.x GitLab repository integration added the GitLab trigger path on the same shared repository criteria model. Live Jira polling plus Flow-owned notification output are finalized in the release sign-off slice described below.

* **Migrations** — `0052_add_jira_trigger_and_webhook_criteria.py` revises `0058`, `0054_add_github_trigger_instance.py` introduces GitHub trigger rows, and `0097_gitlab_repository_integration.py` introduces GitLab repository integration/trigger rows. The later 070fix sweep removes trigger-owned scheduling tables.
...
* **Trigger criteria registry** — Email, Webhook, Jira, GitHub, and GitLab trigger setup/detail surfaces use trigger-aware helper controls while preserving raw JSON editing. Webhook keeps the JSONPath payload tester on the detail page.
...
* **Hub/Watcher restructure** ([`8dc1880`](https://github.com/iamveene/Tsushin/commit/8dc1880)) — Wake Events moved under Watcher at `/wake-events`; the old Hub route remains only as a compatibility redirect. Watcher landing now keeps Wake Events + Continuous Agents in the same in-page tab strip, while the standalone URLs remain available for deep links. Hub is split into **Channels** (WhatsApp/Telegram/Slack/Discord/Playground), **Triggers** (Email/Webhook/Jira/GitHub/GitLab), and **Developer Tools** (Shell Command Center, Sandboxed Tools, and GitHub/GitLab repository connections); legacy communication-tab URLs coerce to Channels.
* **Trigger-owned scheduling removed entirely** ([`b587755`](https://github.com/iamveene/Tsushin/commit/b587755)) — alembic 0071 drops the old scheduling table. Routes, runtime channel module, frontend pages, visual picker, tests — all deleted. Cron utilities preserved as `backend/services/cron_preview_service.py` for Flow scheduled-kind reuse. `TriggerKind` covers event sources (`email | webhook | jira | github | gitlab`).
...
This wraps the 5-wave Triggers↔Flows Unification. The entire B+C+A scope ships on 0.7.0 as the user originally directed; three env-var gates (`TSN_FLOWS_TRIGGER_BINDING_ENABLED`, `TSN_FLOWS_AUTO_GENERATION_ENABLED`, `TSN_FLOWS_BACKFILL_SUPPRESS_LEGACY`) all default-off so the release ships byte-identical to 0.6.x; operators flip them per tenant in staging then production. The deployment ladder lives in the release runbook (still to be authored as `.private/0.7.0_RELEASE_RUNBOOK.md`).

### 2.20 v0.7.x IA reshape — Watcher consolidation + Studio Continuous Agents + auto-flow editor surfacing (2026-05-07)

Post-0.7.0 IA + UX cleanup driven by feedback that the agent-runtime surfaces were scattered, system-managed flows were a black box, and `Watcher Monitor` / `Continuous Agent` / `Continuous Agents tab` collectively named the same thing in three different places. Backend semantics unchanged; everything in this section is presentation-only.

* **Watcher tab consolidation (`frontend/app/page.tsx`)** — `WatcherTab` enum narrowed from 11 entries to **7 top-level**: `dashboard | graph | agents | flows | security | channel-health | billing`. The five agent-runtime surfaces (`continuous-agents`, `wake-events`, `conversations`, `team-runs`, `communication`) collapsed into a new `AgentsSubTab` enum nested under the `agents` top-level tab. `continuous-agents` is the default sub-tab landing because it's the inventory surface (the others are run-history surfaces consulted on-demand).
* **Studio Continuous Agents tab (`frontend/components/studio/StudioTabs.tsx`, `frontend/app/studio/continuous-agents/page.tsx`)** — Studio sub-nav gains a new `Continuous Agents` entry between Personas and Teams. The new page re-mounts `ContinuousAgentsPage` from `/continuous-agents` inside the Studio shell so creation lives next to Agents and Teams (configure surface) while observability stays in Watcher → Agents (monitor surface). The dual mount is intentional and the two pages stay in lockstep — push behavior into the underlying component as a prop instead of forking.
* **3-way kind chooser (`frontend/components/studio/CreateChooserModal.tsx`)** — "What do you want to build?" — Agent / Continuous Agent / Team side-by-side cards with one-paragraph orientation, three-to-four bullets, an example use case, and a routing CTA. Opens from a "Compare options" link on the Studio Agents page next to the existing SplitButton, which now also includes the **Continuous Agent** dropdown option (previously hidden — operators had to navigate to Watcher to discover CA existed). Default routing for Continuous Agent is `/studio/continuous-agents?new=1`, which the existing `?new=1` deep-link plumbing on the CA page picks up to auto-open the setup modal.
* **Auto-flow editor reframing (`frontend/app/flows/page.tsx`, `frontend/components/flows/SourceStepConfig.tsx`, `frontend/components/flows/StepSamplePreview.tsx`)** — five fixes that make system-managed flows readable instead of opaque:
  * `EditFlowModal` prepends a system-managed banner with a deep link back to the Hub trigger detail page (link reconstructed from the source step's `config_json.trigger_instance_id` since `FlowDefinition` doesn't surface it directly).
  * `SourceStepConfig` adds a kind-specific Trigger Configuration card showing JQL/project_key (Jira), inbox/search_query (Email), repo/events/branch/path/author filters (GitHub), project_path/events/branch/path/author filters (GitLab), plus a `trigger_criteria.filters` JSON preview and an "Edit in Hub" deep link.
  * Gate step on system-managed flows prepends an upstream-filter callout explaining the trigger's criteria already gated the event upstream — anything added on the gate is a *secondary* filter and an empty gate is the canonical default.
  * Default-agent conversation step hides outbound-message fields (channel, recipient, initial prompt) when `flowIsSystemOwned` is true, replacing them with a one-line "Inbound step" note. Conversation Objective + Max Turns + Timeout + Output Alias + Agent picker stay (they apply to inbound).
  * New `StepSamplePreview` component renders an expandable "Sample data this step receives" panel on every non-source step in system-managed flows. Fetches the most-recent wake event for the bound trigger (via `getWakeEvents` + `getWakeEventPayload`) and shows the raw JSON payload + a count badge of `{{source.payload.X}}` references already in the step's config. Walks the step's `config_json` recursively to extract those refs (regex `/\{\{\s*source\.payload\.([^}\s]+)\s*\}\}/g`).

  Wiring: a new `flowIsSystemOwned: boolean` prop is threaded through `EditFlowModal → EditableStepBuilder → EditableStepConfigForm` and into `StepConfigForm` in the new-flow path (defaults to false). Default false means user-authored flows render exactly as before — no regression. The flag is sourced from `FlowDefinition.is_system_owned`, populated by the auto-flow generator's `is_system_owned=true` setting.
* **WiredTeamsCard for email triggers (`frontend/components/triggers/sections/OutputsSection.tsx`)** — pre-fix the email branch hid the WiredTeamsCard with a wrong comment ("the team API rejects email/gmail kinds"). The team API actually accepts `gmail` (`agent_team_api_service._validate_team_trigger_instance` maps `TeamTriggerKind.GMAIL → EmailChannelInstance`). Email triggers now show Wired Agent Teams at parity with Jira.
* **Parallel-fire / flow-only pill toggle (`frontend/components/triggers/sections/WiredFlowsCard.tsx`)** — the previous "Suppress default agent" checkbox is replaced by a clearer pill toggle: amber **● Parallel fire** (default, `suppress_default_agent=false` — both flow + legacy CA path fire) ↔ emerald **● Flow-only** (`suppress_default_agent=true` — only the flow fires). Hover tooltip explains both states. Toast messages reworded to mirror the pill copy. Underlying API call (`updateFlowTriggerBinding`) and persisted field unchanged.
* **In-page rename: `Watcher Monitor` → `Continuous Agent`** — touched the CA list page (h1, breadcrumb, button, error/loading copy, fallback names), the `[id]` detail page, the setup modal (title + tagline + error), the SubscriptionEditor card heading, and the agent-vs-flow explainer's bullet copy. Backend model name (`ContinuousAgent`) unchanged — UI rename only.

PR #56 (gate canonical schema + on_failure round-trip) and PR #61 (everything in this section) merged into 0.7.x. New tests under `backend/tests/test_flow_binding_service_gate_schema.py` and `backend/tests/test_routes_flows_step_failure_roundtrip.py` lock the contracts surfaced by these fixes; full flow + trigger backend test surface (54 tests) green.
...
* **`STEP_OUTPUT_FIELDS.source`** — `frontend/lib/stepOutputVariables.ts`. Adds the `source` namespace with 7 variables (`payload`, `trigger_kind`, `instance_id`, `event_type`, `dedupe_key`, `occurred_at`, `wake_event_id`) so downstream steps' Insert Variable dropdown shows `{{source.*}}` when the flow has a Source step.

### 2.16 v0.7.0 Triggers↔Flows Unification — Wave 3 (dispatch fork + email/webhook fork retirement)

Third merge wave. Lands the additive dispatch fork in `TriggerDispatchService` and the full retirement of the email + webhook standalone trigger detail pages into the shared shell.

* **Dispatch fork** — `backend/services/trigger_dispatch_service.py` now reads `flow_trigger_binding` rows for the (kind, instance_id) of every wake event and enqueues one `flow_run_triggered` MessageQueue item per active binding alongside the existing ContinuousRun enqueue. The wake payload is nested under `trigger_context["source"]` so the Wave 2 `SourceStepHandler` exposes `{{source.payload.*}}` / `{{source.trigger_kind}}` / `{{source.event_type}}` / `{{source.dedupe_key}}` / `{{source.occurred_at}}` / `{{source.wake_event_id}}` to every downstream step. The fan-out happens after the existing ContinuousRun enqueue, never aborts dispatch on errors, and uses a single payload read (`_read_payload_ref`) shared across all bindings to keep webhook reply latency below 100ms p99 even with 10 bindings.
...
* **In-scope defects fixed during the refactor** (per the user's directive that pre-existing problems uncovered by current work must be fixed in the current wave, not punted):
  - **Webhook secret rotation now reachable from the UI.** `api.rotateWebhookSecret` was implemented in `frontend/lib/client.ts` but had no UI affordance. The Security card on the unified webhook source view now exposes a Rotate Secret button with a confirm() prompt and a 12-second toast that displays the plaintext secret once.
  - **Webhook status pill turns red when `circuit_breaker_state === 'open'`.** The shared shell's `statusClass` previously did not handle webhook (the standalone fork did). Now wired so an open circuit breaker produces a visual red status pill.
  - **`EditFlowModal canWriteFlows is not defined`** — the variable was declared inside `FlowsPage` scope only, but `EditFlowModal` (separate component) referenced it at lines 4690-4696. The modal now derives `canWriteFlows` locally via its own `useAuth()` call. (Wave 1 fixed the registry-level bug; Wave 3 catches the remaining scope-level reference.)

### 2.15 v0.7.0 Triggers↔Flows Unification — Wave 2 (Source step + flow protection + 3-section UI)

Second merge wave. Lands the canonical `source` step type, the `triggered` execution method, the flow-protection enforcement, and the three-section refactor of the trigger detail Overview tab.
...
* **`'triggered'` execution method** — added to `VALID_EXECUTION_METHODS` in `backend/api/routes_flows.py:874`. The Flow create UI exposes it as an execution method that first selects an existing Email/Gmail, Jira, GitHub, GitLab, or Webhook Hub trigger, then writes both the Source-step config and `flow_trigger_binding`.
...
### 2.14 v0.7.0 Triggers↔Flows Unification — Wave 1 (foundations)
...
* **Three default-OFF env-var gates** (`backend/config/feature_flags.py`):
  * `TSN_FLOWS_TRIGGER_BINDING_ENABLED` — Wave 3 dispatch fork: `TriggerDispatchService.dispatch` queries `flow_trigger_binding` and enqueues FlowRuns alongside ContinuousRuns.
  * `TSN_FLOWS_AUTO_GENERATION_ENABLED` — Wave 4 auto-Flow per trigger and notification-toggle write-through onto the auto-flow's Notification node.
...
5. On first login an **onboarding tour** auto-opens. It now uses the v0.7.0 getting-started path instead of the old v0.6.0 showcase: AI providers, channels vs triggers, GitHub/GitLab repository connections, skills, memory and knowledge, Watcher, Studio, Hub, flows, Playground, optional voice setup, Sentinel, trigger readiness, and final next steps. Setup-oriented steps include direct links or guided wizard actions to the relevant Hub, Studio, Flows, Playground, or setup surface. The tour pauses while another blocking editor modal is open, then resumes when that modal closes.
6. The tour highlights mandatory next steps: **connect a communication channel** (WhatsApp, Telegram, Slack, or Discord) via Hub > Channels to enable agent messaging. Signed HTTP events use the separate Webhook Trigger path in Hub > Triggers, and repository automation reuses GitHub/GitLab connections under Hub > Developer Tools.
...
| `code_repository` | Code Repository | tool | Search/read/act on a connected source-control system. v0.7.x supports GitHub and GitLab repository providers. Missing `provider` config defaults to GitHub for existing agents. Read actions are on by default (`search_repos`, `list_pull_requests`, `read_pull_request`, `list_issues`, `read_issue`); write actions are off by default (`create_issue`, `add_pr_comment`, `approve_pull_request`, `request_changes`, `merge_pull_request`, `close_pull_request`, `close_issue`). GitLab write actions currently fail closed until explicitly enabled by a future provider release | `code_repository_skill.py` |
...
**Single source of truth for skill_type validation (2026-05-16).** Save endpoints (`POST /api/v2/agents/{id}/builder-save`, `PUT /api/v1/studio/agents/{id}`) used to maintain their own duplicate `SKILL_METADATA` dicts inline. That broke every time a new skill landed in `SkillManager._register_builtin_skills` but only one of the dicts got updated, producing `invalid skill_type: <name>` errors on save for agents that already carried the missing skill_type. As of 2026-05-16 the metadata + validator live in `backend/constants/skill_metadata.py`. `get_valid_skill_types()` derives the allow-list from `SkillManager.get_registered_skill_types()` union legacy aliases (`calendar`, `asana`, `email`), so adding a new skill in `skill_manager.py` automatically opens it for save validation. Drift is guarded by `backend/tests/test_skill_registry_integrity.py`.
...
Four v0.7.x skills follow a shared **capability-gating** contract — `ticket_management` (Jira), `code_repository` (GitHub/GitLab), `password_vault` (1Password), and `gmail` with its v0.7.0 granular send/reply/draft capabilities. They share four design properties:
...
| `code_repository` (GitHub/GitLab) | `search_repos`, `list_pull_requests`, `read_pull_request`, `list_issues`, `read_issue` | `create_issue`, `add_pr_comment`, `approve_pull_request`, `request_changes`, `merge_pull_request`, `close_pull_request`, `close_issue` | `code_repository_skill.py` |
...
The flow editor surfaces a collapsible **Variable Reference** panel under every templatable step-config field. It auto-populates with the output variables of every preceding step in the flow, plus per-trigger-kind deep payload paths when the source is a Jira/Email/GitHub/GitLab/Webhook trigger.
...
Flows minted by `ensure_system_managed_flow_for_trigger()` (4-node Source/Gate/Conversation/Notification chain, `is_system_owned=true`) display a per-kind pill in the flows list and editor header — "Jira Trigger" (blue), "Email Trigger" (emerald), "GitHub Trigger" (violet), "GitLab Trigger" (orange), and "Webhook Trigger" (cyan). The badge tooltip explains: *"Auto-generated from <kind> trigger — editable, but not deletable. Delete the trigger to remove this flow."* The Delete button on the row is disabled to match. User-authored flows show no badge.
...
A single wizard creates event triggers for Email, Webhook, Jira, GitHub, and GitLab. It replaces the legacy trigger setup entry-points so operators see the same flow regardless of which trigger they're creating. The wizard is reachable from:
...
|---|---|---|---|
| 1 | **Kind** | yes | 5-tile `role="radiogroup"` (Email / Webhook / Jira / GitHub / GitLab) with `role="radio"` + `aria-checked`. Skipped when an entry-point passes `initialKind`. |
| 2 | **Source** | per-kind | Per-kind input grid: Jira (linked Hub Tool API connection + project key + JQL + poll interval + default agent + name), GitHub (linked Hub repository connection + owner + repo + events checklist + default agent + name), GitLab (linked Hub repository connection + full project path + events checklist + default agent + name), Email (Gmail account + saved query / label / from-address filters + default agent + name), Webhook (name + slug mode + callback URL + IP allowlist + rate limit). All inputs have `htmlFor`/`id` associations. Continue is gated on per-kind required fields. |
| 3 | **Criteria** | per-kind | Jira: read-only JQL preview + "Test Query" button. GitHub/GitLab: full repository event/action/filter builder via `CriteriaBuilder` with raw envelope JSON preview + "Test against sample payload" dry-run. Email: saved-query test against the connected mailbox. Webhook: JSONPath payload criteria tester. |
...
The auto-flow ID is surfaced on every trigger Read schema (`JiraTriggerRead.auto_flow_id`, `EmailTriggerRead.auto_flow_id`, `GitHubTriggerRead.auto_flow_id`, `GitLabTriggerRead.auto_flow_id`, `WebhookTriggerRead.auto_flow_id`). The Confirmation step reads it and renders the Wired Flow card; if no auto-flow was generated (older trigger, or feature flag off), the card falls back to a "Wire a custom Flow" CTA that deep-links to `/flows?source_trigger_kind=<kind>&source_trigger_id=<id>` (the Wave 4 deep-link prefill path).

Trigger default-agent edits keep the generated system-managed Flow aligned: the trigger `default_agent_id`, the FlowDefinition default agent, and the generated Conversation node's `agent_id` are synchronized for Email, Jira, GitHub, GitLab, and Webhook updates.
...
- `EmailTriggerWizard.tsx` — was the standalone email-only create/edit wizard. Email trigger editing now happens on `/hub/triggers/email/{id}` through the shared `TriggerDetailShell`, matching Webhook, Jira, GitHub, and GitLab.

**A11y:** wizard inputs use `htmlFor`/`id` associations. Kind-picker tiles in step 1 are a `role="radiogroup"`. GitHub/GitLab event/action chips are in `role="group"` + `aria-pressed`.
...
- **Review team:** creates a Team with **Coordinator**, **Reviewer**, and **Merge Readiness** roles. The active Team trigger binding is the primary executor; the generated Flow is still created and linked as the editable deterministic output surface, but its trigger binding stays inactive so the same PR/MR is not reviewed twice. The wizard accepts `notify_contact_id` for the Team completion notification target, defaults omitted values from the current user's active reachable contact mapping, and treats explicit null as no team notification. Legacy generated review teams with no stored directive are backfilled from the creator's mapped reachable contact, or from an active tenant owner's mapped reachable contact when the historical creator has no usable mapping.
...
The backend entrypoint is `POST /api/wizards/repository-automation`. It accepts `provider`, `integration_id`, provider-specific repository target (`repo_owner` + `repo_name` or `project_path`), `template_id`, optional `existing_trigger_id`, repository events and filters, criteria, names, routing preferences, and optional `notify_contact_id` for review-team completion delivery. The response returns created/reused `integration`, `trigger`, `flow`, optional `team`, `agents[]`, `bindings[]`, `links`, `routing_mode`, and optional `provider_webhook_setup`; the optional `team` payload includes `notification_contact_id` and `notification_contact_name`.

Review-team completion delivery is Team-owned, not the inactive generated Flow's Notification node. The directive is persisted on the Agent Team for the team-run completion path, Team APIs strip that directive from editable description text while returning explicit notification metadata, and the generated Flow edit modal reads wired team bindings so operators can see the completion target from the Flow editor.
...
**Source:** `backend/api/routes_gitlab_triggers.py`, `backend/api/routes_gitlab_inbound.py`, `backend/channels/gitlab/trigger.py`

GitLab triggers receive token-gated GitLab.com project webhook deliveries and dispatch matching events to continuous-agent wakeups, bound Flows, and Agent Teams. They select a tenant-owned Hub GitLab integration; trigger-local GitLab PATs are not accepted. The same Hub connection can also back the Code Repository skill, keeping provider credentials centralized.

**API surface:**
- `GET/POST /api/triggers/gitlab`
- `GET/PATCH/DELETE /api/triggers/gitlab/{id}`
- `POST /api/triggers/gitlab/{id}/inbound`
- `POST /api/triggers/gitlab/test-criteria`
- `POST /api/triggers/gitlab/{id}/test-criteria`

**Stored config:** linked `gitlab_integration_id`, full GitLab project path (`group/subgroup/project`), encrypted webhook-secret preview, event filters, branch/path/author filters, trigger criteria, default agent, and delivery/activity cursors. Credential tests happen on the Hub repository integration surface.

**Inbound contract:** GitLab sends `X-Gitlab-Token`, `X-Gitlab-Event`, and usually `X-Gitlab-Event-UUID`. The route validates the token against the encrypted trigger secret, applies project/event/branch/path/author filters, dedupes on the UUID or a body hash fallback, and dispatches a normalized repository payload via `TriggerDispatchService`.
...
Transcription is performed by the `audio_transcript` skill (Source: `backend/agent/skills/audio_transcript.py`) using one of three modes: explicit OpenAI Whisper (`asr_mode='openai'`), a specific pinned local Speaches/OpenAI-Whisper instance (`asr_mode='instance'`), or Google Gemini multimodal (`asr_mode='gemini'`, default `gemini-3.5-flash`, source `backend/hub/providers/gemini_asr_provider.py`). Gemini ASR reuses the tenant's Gemini Provider Instance (same credential as the Gemini LLM); audio under 20 MB is sent inline, larger files automatically fall through to the Gemini Files API. When a local ASR instance is pinned and unavailable, the skill fails closed with the local-ASR error and does not fall back to OpenAI, preserving the operator's privacy/cost choice; the same fail-closed contract applies to the Gemini branch. OpenAI Whisper is used only when `asr_mode='openai'` or no other provider is pinned. Pinned local ASR configs may now include `transcription_prompt` and `hotwords`; the runtime normalizes those fields, passes `prompt` / `hotwords` through `ASRRequest`, and sends them to Speaches multipart transcription requests alongside `language` and `vad_filter`. OpenAI cloud receives only the optional prompt and remains an explicit mode/reference path, not a hidden fallback. Long local ASR calls release any open PostgreSQL transaction before waiting on the provider so long voice notes do not trip the backend idle-in-transaction timeout. Transcript-only responses and transcription failures are delivered through the router's direct skill-output path across normal, active-thread, and active-conversation routes so media-only turns do not fall into empty-message AI processing. Skill is `execution_mode="special"` (media-triggered).
...
Auto-provisioned ASR containers are tenant-scoped by deterministic name/DNS alias plus Docker labels: `tsushin.service=asr`, `tsushin.tenant=<tenant_id>`, `tsushin.instance_id=<asr_instance.id>`, and `tsushin.lifecycle=auto-provisioned`. Startup reconciliation compares those labels with active `ASRInstance` rows, removes labeled ASR containers with no matching active row, and refuses lifecycle operations when a row points at a container owned by another tenant or instance. Speaches containers default to CPU-oriented `WHISPER__COMPUTE_TYPE=int8` and keep the selected model warm with `WHISPER__TTL=-1` (`STT_MODEL_TTL=-1` is also sent for compatibility). Updating an active auto-provisioned ASR instance's `default_model` is runtime-affecting: the API updates the row and reprovisions the same container name, port, and model-cache volume so local model promotion can happen without losing downloaded HuggingFace artifacts. Kokoro and Ollama containers follow the same lifecycle label convention and lifecycle/log/status actions also verify `tsushin.tenant` + `tsushin.instance_id` before managing a container.
...
### 20.4.3 GitLab (GitLab.com + code_repository skill)

**Sources:** `backend/services/gitlab_integration_service.py`, `backend/api/routes_gitlab_integrations.py`, `backend/hub/gitlab/gitlab_repository_service.py`, `backend/api/routes_gitlab_triggers.py`, `backend/channels/gitlab/trigger.py`, `frontend/app/hub/page.tsx` (GitLabIntegrationModal).

GitLab is a polymorphic subclass of `HubIntegration` that provides GitLab.com repository access for the `code_repository` skill and GitLab repository triggers. v1 targets `https://gitlab.com/api/v4`; self-managed GitLab and OAuth are out of scope.

**Hub UI:** Hub → Developer Tools → Add Repository Connection → GitLab. The modal asks for:

| Field | Required | Notes |
|---|---|---|
| Personal/project access token | yes | Stored encrypted with the API-key encryption key. Preview shown as `<first 4>...<last 4>` only. |
| Default project path | optional | Full path such as `group/subgroup/project`; used when a read action omits the project. |
| Enabled | yes | Pauses or enables use of this shared connection. |
...
| Endpoint | Description |
|---|---|
| `POST /api/hub/gitlab-integrations` | Create. Normalizes default project paths and stores encrypted token material. |
| `GET /api/hub/gitlab-integrations` | List for the tenant; PAT preview masked. |
| `PATCH /api/hub/gitlab-integrations/{id}` | Update name / token / default project path / active state. A blank token leaves the stored token unchanged. |
| `DELETE /api/hub/gitlab-integrations/{id}` | Returns `409` when referenced by any `AgentSkillIntegration(skill_type='code_repository')` or FK-linked `GitLabChannelInstance`. |
| `POST /api/hub/gitlab-integrations/test-connection` | Raw-creds dry-run. Uses GET-only GitLab.com calls and returns connection status. |
| `POST /api/hub/gitlab-integrations/{id}/test-connection` | Saved-creds dry-run for already-stored integrations. |

**Code Repository skill:** the same `repository_operation` tool chooses GitLab when the agent skill config stores `provider: "gitlab"`. GitLab maps `pull_request` actions to merge requests and `pr_number` to MR IID. Read actions (`search_repos`, `list_pull_requests`, `read_pull_request`, `list_issues`, `read_issue`) use URL-encoded nested project paths. GitLab write actions remain fail-closed and are not enabled by default.

**Validation note:** live read validation on 2026-05-19 used `mvpenha/questnexus` merge request `!1`. The Code Repository skill listed the open MR and read it through `pr_number=1`, confirming GitLab's MR IID mapping. GitLab project creation requires a token with `api` scope; the product path documented here keeps GitLab write actions disabled/fail-closed.

**Trigger criteria envelope:** GitLab uses the shared repository criteria model. Merge request webhook events normalize to canonical `pull_request` criteria for backward compatibility while provider-prefixed dispatch payloads retain GitLab-specific names such as `gitlab.merge_request`, `gitlab.push`, `gitlab.issue`, `gitlab.note`, `gitlab.tag`, and `gitlab.pipeline`.

### 20.4.4 Password Vault (1Password provider)
...
3. **Voice & transcription settings** — language first (shared between TTS + STT), then:
   ...
   for transcription, an **ASR backend** selector with *OpenAI Whisper* and *Pin a specific local instance*, plus a **Remember audio transcripts** toggle. Missing `remember_transcript` config is treated as enabled so older agents keep recording transcripts into conversation context by default. The local-instance options are sourced from `GET /api/asr-instances`.
...
- Runtime diagnostics from Docker inspect when available, including restart count and OOM-style exit context.
...
|---|---|---|---|---|---|---|
| `speaches` | `ghcr.io/speaches-ai/speaches` | `POST /v1/audio/transcriptions` | Bearer (encrypted Fernet token per instance) | `Systran/faster-whisper-small` | `/home/ubuntu/.cache/huggingface` (image runs as `ubuntu` uid 1000) | OpenAI-compatible multilingual engine. Recommended/default memory is **4 GB** so model preload and transcription have headroom; lower limits can produce OOM-style startup or request failures. CPU deployments set `WHISPER__COMPUTE_TYPE=int8` and `WHISPER__TTL=-1`. `PRELOAD_MODELS` only loads from disk; first-run model download is triggered by `POST {base_url}/v1/models/{model_id}` after provisioning. |
...
**Hub Provider Wizard ASR modality.** `frontend/components/provider-wizard/steps/StepModality.tsx` exposes "Speech-to-Text (Audio in)" as a 4th modality. Picking ASR → Cloud surfaces only `openai` (Whisper API; reuses the saved OpenAI key) and skips credentials/test steps because there is no separate provider row to create. Picking ASR → Local surfaces both `openai_whisper` (new) and `speaches`, then routes through the standard container provision step before calling `POST /api/asr-instances` with `auto_provision=true`. The wizard keeps Speaches on the shared **4 GB** memory recommendation/default; `openai_whisper` keeps its smaller CPU-oriented default. Updating runtime limits on an auto-provisioned ASR instance reprovisions the container with the same model-cache volume so model downloads are preserved.

**Per-agent assignment.** The `audio_transcript` skill exposes only two ASR modes: `openai` (cloud Whisper API) and `instance` (pin a specific tenant-owned ASR instance via `asr_instance_id`). The Audio Agents Wizard and Agent Skills Manager surface this choice; both list every active tenant ASR instance so the user can pick which one this agent uses. The same shared transcript UI also persists `remember_transcript`, defaulting to enabled when the key is absent, so operators can decide whether audio transcripts become conversation context for that agent. Local instance mode also exposes optional prompt/context and hotword fields, persisted as `transcription_prompt` and `hotwords`; Speaches receives those as multipart `prompt` / `hotwords`, while OpenAI cloud receives only the optional prompt when cloud mode is explicitly selected. When the pinned local instance is Speaches, the UI also exposes a Speaches VAD toggle backed by `audio_transcript.config.vad_filter`; set it to disabled when Speaches returns `empty_transcription` because its VAD removed a real voice note.
...
**No silent OpenAI fallback when a local instance is pinned.** When `asr_mode == 'instance'` the operator explicitly chose a local Whisper or Speaches instance for privacy/cost reasons. Falling back to OpenAI on local failure violated that intent: it routed audio to a third-party cloud invisibly, leaked the user's voice data, and hid the real failure mode behind misleading 401 noise. A pinned local-instance failure now surfaces directly with the local provider's error and no OpenAI call is made. OpenAI Whisper remains available when the operator explicitly sets `asr_mode == 'openai'`. Transient provider errors are retried on the local path, and OOM-like container failures keep their local diagnostic detail so the operator can raise the ASR memory limit instead of chasing cloud credential state.
...
* **Public ingress (BUG-712, BUG-723):** Caddy now has an explicit `handle /metrics { reverse_proxy <stack>-backend:8081 }` block in both `proxy/Caddyfile` and the `tsushin_routes` snippet generated by `install.py`, so the Next.js auth middleware no longer 307s `/metrics` to `/auth/login` when the proxy is in front. Pre-fix, scrapes against the public hostname silently failed; post-fix, `https://<host>/metrics` proxies straight to the stack-scoped backend (which then enforces the bearer token if `TSN_METRICS_SCRAPE_TOKEN` is set).
...
| `BACKEND_INTERNAL_URL` | `http://${TSN_STACK_NAME:-tsushin}-backend:8081` | Backend origin used by the frontend proxy layer inside Docker: baked into Next fallback rewrites at image build time and read by the auth proxy at runtime. Keep stack-scoped on the shared `tsushin-network`. | — | `docker-compose.yml:271`, `frontend/Dockerfile:43-53`, `frontend/next.config.mjs:24`, `frontend/app/api/auth/[...path]/route.ts:21` |
| `INTERNAL_API_URL` | `http://${TSN_STACK_NAME:-tsushin}-backend:8081` | Server-side frontend fallback for auth/status requests. | — | `docker-compose.yml:272`, `frontend/app/auth/login/page.tsx:18`, `frontend/app/api/auth/[...path]/route.ts:22` |
...
| **Hub** | Tenant-facing integration marketplace, split into roles: **Channels** (WhatsApp/Telegram/Slack/Discord/Playground), **Triggers** (Email/Webhook/Jira/GitHub/GitLab), **Developer Tools** (Shell Command Center, Sandboxed Tools, and GitHub/GitLab repository connections), **Tool APIs** (Jira, 1Password Password Vault, search/flight providers), and **Local Services** (auto-provisioned Whisper/Speaches, Kokoro, Ollama). |
...
| **Trigger** | A non-conversational event source (Email/Gmail, Webhook, Jira, GitHub, GitLab) that fans wake events out to bound flows, continuous-agent subscriptions, and matching agent-team triggers in parallel. Each trigger mints a system-managed FlowDefinition (Source → Gate → Conversation → Notification) on creation. |
