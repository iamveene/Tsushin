...
Review-team templates also accept `notify_contact_id`. The service validates that the contact is active, tenant-scoped, and WhatsApp-reachable, then writes the team completion directive used by the Agent Team runtime. If the request omits `notify_contact_id`, the service defaults to the current user's `user_contact_mapping` when one exists.
...
The backend entrypoint is `POST /api/wizards/repository-automation`. It accepts `provider`, `integration_id`, provider-specific repository target (`repo_owner` + `repo_name` or `project_path`), `template_id`, optional `existing_trigger_id`, repository events and filters, criteria, names, routing preferences, and optional `notify_contact_id` for review-team notifications. The response returns created/reused `integration`, `trigger`, `flow`, optional `team` including notification contact metadata when configured, `agents[]`, `bindings[]`, `links`, `routing_mode`, and optional `provider_webhook_setup`.
