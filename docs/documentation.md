...
* **Regression coverage** — `backend/tests/test_gmail_send_phase3_checkpoint.py` covers send, reply, draft, and scope-gating behavior. `backend/tests/test_gmail_send_phase3_live_gate.py` is an opt-in root-only live proof that stays skipped unless `TSN_RUN_GMAIL_PHASE3_LIVE_GATE=1` is set; once enabled, it fails if the fixture still has only `gmail.readonly + gmail.send`, and the optional agent-chat/API proof additionally requires `TSN_RUN_GMAIL_AGENT_CHAT_LIVE_GATE=1` plus API endpoint/token/agent env vars. The current checkpoint proves send/reply plus Sent visibility, but draft and full API/agent-chat completion remain blocked until the Gmail fixture is reauthorized with `gmail.compose` or a broader Gmail write scope.
...
* **Evidence** — see `docs/qa/v0.7.0/phase-3-email-trigger-triage-summary.md` for the release-note boundary and remaining risks. The opt-in live proof is `backend/tests/test_email_trigger_phase3_live_gate.py` and remains skipped unless `TSN_RUN_EMAIL_PHASE3_LIVE_GATE=1` is set after Gmail compose reauthorization.
