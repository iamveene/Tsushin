...
- **Files:** `backend/api/v1/routes_agents.py`, `backend/api/routes_agents.py`
- **Description:** The tenant product and internal agent APIs accept `model_provider="vertex_ai"`, but the public API v1 schema still constrains `model_provider` to `openai|anthropic|gemini|ollama|openrouter|groq|grok|deepseek`. Generated clients from `/api/v1/openapi.json` therefore type Vertex AI as invalid even though the release surface otherwise supports it.
- **Evidence:** `.private/qa/ubuntu-vm-20260410/reports/api_v1_openapi.json`; `.private/qa/ubuntu-vm-20260410/reports/public_api_schema_report.json`; `backend/api/v1/routes_agents.py:100,163` still exclude `vertex_ai` while `backend/api/routes_agents.py:190,233` include it.
- **Remediation:** Add `vertex_ai` to the public API v1 request-model patterns and regenerate the authoritative `/api/v1/openapi.json` surface so generated SDKs match the supported runtime providers.
...
- **Files:** `frontend/app/agents/page.tsx`, `frontend/components/AgentConfigurationManager.tsx`, `frontend/components/playground/ConfigPanel.tsx`
- **Description:** Fresh-install agent-management surfaces still call `/api/ollama/health` against the Next.js frontend origin instead of the backend API origin, producing `404` console noise on a healthy install. The backend Ollama endpoint is reachable and the VM runtime was configured correctly, but the UI probes the wrong origin on `/agents` and related config panels.
- **Evidence:** `.playwright-cli/console-2026-04-10T20-08-47-457Z.log` recorded `GET http://10.211.55.5:3030/api/ollama/health 404`; live API health checks against the backend and direct Ollama tag discovery both succeeded during the same session.
- **Remediation:** Route these checks through the authenticated backend base URL (or a real frontend proxy) consistently instead of relying on a nonexistent Next.js `/api/ollama/health` path.
...
- **Description:** There is no dedicated `/agents/communication` page, so Next.js resolves that URL through the dynamic agent-detail route. `params.id` becomes `"communication"`, `parseInt()` yields `NaN`, the page requests `/api/agents/NaN`, raises the alert `Failed to load agent details`, and redirects back to `/agents`. The underlying A2A backend path and Graph View both worked during the audit, so this is a route/UI regression rather than another backend A2A failure.
- **Evidence:** `.playwright-cli/console-2026-04-10T20-10-59-074Z.log` shows the `422` from `/api/agents/NaN`; `output/playwright/ubuntu-vm-graph-view-a2a-20260410.png` confirms the same install rendered the A2A graph edge successfully.
- **Remediation:** Add a real `/agents/communication` route (or redirect it intentionally) and guard the dynamic agent page against non-numeric `params.id` before calling the API.
...
- **Files:** `backend/agent/skills/custom_skill_adapter.py`, `backend/agent/skill_manager.py`, `backend/agent/agent_service.py`
- **Description:** An instruction custom skill created during the VM audit tested successfully through `POST /api/custom-skills/{id}/test`, but invoking the same assigned skill from Playground chat returned the raw `instructions_md` text instead of the expected runtime output. Execution history also only showed the direct `/test` invocation, which suggests the Playground/runtime path is still bypassing actual custom-skill execution for this case.
- **Evidence:** The audit skill `Audit Echo Skill` returned `AUDIT_SKILL_OK` through `/api/custom-skills/1/test`, then Playground chat answered with `When this skill is executed, respond with exactly AUDIT_SKILL_OK and nothing else.` and did not create a matching execution-history row.
- **Remediation:** Ensure the chat/runtime path resolves assigned instruction custom skills through the same execution adapter that now powers the `/test` endpoint, rather than exposing the raw instructions back to the user.
...
- **Files:** `backend/services/slash_command_service.py`, `backend/services/shell_command_service.py`, `backend/agent/memory/tool_output_buffer.py`
- **Description:** The fresh VM audit queued `/shell whoami`, completed the linked shell command through a live Ubuntu beacon, and verified the command result via the Shell Command Center API. Despite that, `/inject list`, `/inject <command_id>`, and `/inject shell` could not see the finished shell execution even though the slash-command UX explicitly tells the user to use `/inject` next. The end-to-end shell workflow is therefore still broken after command dispatch succeeds.
- **Evidence:** `/shell whoami` returned `Command Queued` with command id `3c2efc22-d3bb-4c8a-9fc7-093c5a3ba39f`; the beacon later reported `stdout=root` and `status=completed`; subsequent `/inject list` omitted the shell execution and `/inject 3c2efc22-d3bb-4c8a-9fc7-093c5a3ba39f` returned `No ... executions found.`
- **Remediation:** Buffer completed shell-command executions into the same injectable execution store that `/inject list` reads, or update the shell slash-response copy if shell results are intentionally out of scope for injection.
...
- **Files:** `backend/services/project_service.py`, `backend/services/playground_service.py`, `backend/api/routes_projects.py`
- **Description:** Project facts and project knowledge uploads both succeeded on the fresh VM, but chatting inside the project conversation still returned a success envelope with no usable assistant reply when asked about either the stored fact or the uploaded document. This leaves project chat unable to surface the very context it just persisted.
- **Evidence:** The audit project stored fact `audit.favorite_snack=pao de queijo` and uploaded `.private/qa/ubuntu-vm-20260410/reports/project_knowledge.txt` with `status: completed`; follow-up calls to `POST /api/projects/1/conversations/1/chat` returned success with an empty/null assistant response instead of answering from the saved project context.
- **Remediation:** Make project chat fall back to the non-empty Playground/agent answer fields consistently and add a regression that asserts a stored project fact or uploaded project document produces a non-empty assistant response.
...
- **Files:** `backend/api/routes_mcp_servers.py`, `backend/services/toolbox_container_service.py`
- **Description:** Registering the documented stdio MCP server (`uvx mcp-server-fetch`) succeeded, but pressing the normal test path immediately failed with `Container not found for tenant ... Please start it first.` On a fresh install there is no obvious UI action that explains or performs that prerequisite, so the documented stdio validation path still dead-ends before the server can be exercised.
- **Evidence:** `POST /api/mcp-servers/1/test` returned `Connection failed: Failed to verify binary 'uvx' in container: Container not found for tenant ... Please start it first.` even though the saved stdio server configuration itself was valid.
- **Remediation:** Auto-start the tenant toolbox when stdio MCP servers are tested/refreshed, or surface a first-class bootstrap action and guidance from the MCP UI instead of returning a raw container error.
...
- **Description:** The Vertex AI modal exposes `Test Connection` before `Save Configuration`, but the test action always calls `/api/integrations/vertex_ai/test` with an empty body and therefore validates only whatever keys are already stored for the tenant. In practice, typing new values into the modal and pressing `Test Connection` still uses the previously saved configuration, which makes the modal misleading during first-time setup and edits.
- **Evidence:** `frontend/app/hub/page.tsx:1238` posts `{}` to `/api/integrations/vertex_ai/test`; during the live VM setup, the unsaved modal form could be fully populated while the test path still behaved as if required fields were missing until the configuration was saved first.
- **Remediation:** Either disable `Test Connection` until the new values are saved, or send the in-form Vertex AI fields to a draft/test endpoint so the button validates what the user actually entered.
