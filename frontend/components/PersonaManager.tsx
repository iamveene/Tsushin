...
import { BookOpenIcon, ClipboardIcon, SparklesIcon, BotIcon } from '@/components/ui/icons'
...
export default function PersonaManager() {
  ...
  const handleClonePersona = (persona: Persona) => {
    ...
    setUseCustomTone(!!persona.custom_tone)
    setEditingPersona(null) // null = new persona creation
    setShowCreateModal(true)
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-lg text-tsushin-slate">Loading personas...</div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-white">Persona Management</h2>
          <p className="text-sm text-tsushin-slate mt-1">
            Create reusable personas with tone, personality traits, and role-playing characteristics
          </p>
        </div>
        <button
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-lg shadow">
        <div className="px-6 py-4 border-b border-gray-800">
          <h3 className="text-lg font-semibold text-white inline-flex items-center gap-2"><BookOpenIcon size={18} /> Persona Template Library</h3>
          <p className="text-sm text-tsushin-slate mt-1">Pre-built persona templates from the Tsushin team. Clone to customize.</p>
        </div>
          ...
          {systemPersonas.map((persona) => (
            ...
            >
              ...
              <div className="space-y-2 text-sm">
                ...
                {persona.guardrails && (
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-white text-xs italic">{persona.guardrails.substring(0, 100)}{persona.guardrails.length > 100 ? '...' : ''}</span>
                  </div>
                )}
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-gray-700">
                ...
                >
                  <span className="inline-flex items-center gap-1"><ClipboardIcon size={14} /> Clone Template</span>
                </button>
              </div>
            </div>
          ))}
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-lg shadow">
        <div className="px-6 py-4 border-b border-gray-800">
          <h3 className="text-lg font-semibold text-white inline-flex items-center gap-2"><SparklesIcon size={18} /> Custom Personas</h3>
          <p className="text-sm text-tsushin-slate mt-1">User-created personas that can be edited or deleted</p>
        </div>
                ...
                <div className="mb-3">
                  ...
                  {persona.ai_summary && (
                    <div className="mt-2 p-2 bg-tsushin-indigo/10 border border-tsushin-indigo/20 rounded-md">
                      <p className="text-xs text-tsushin-indigo italic inline-flex items-center gap-1"><BotIcon size={12} /> AI Summary: {persona.ai_summary}</p>
                    </div>
                  )}
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {editingPersona?.is_system && (
                  <p className="text-xs text-yellow-400 mt-1">System persona names cannot be changed</p>
                )}
              </div>
                ...
                <div className="space-y-3">
                  ...
                  {!useCustomTone ? (
                    ...
                    <textarea
                      ...
                      placeholder="Describe the tone and communication style (e.g., 'Be warm, friendly, and use emojis when appropriate')"
                    />
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-tsushin-indigo h-24 bg-gray-900 text-white"
                  placeholder="Safety rules and constraints (e.g., 'Never discuss sensitive information', 'Always verify before making changes')"
                />
              </div>
