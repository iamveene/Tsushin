    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4">
          <div className="border border-tsushin-border rounded-lg p-4 bg-blue-50 dark:bg-blue-900/20">
            <div className="text-sm text-tsushin-slate mb-1">Total Conversations</div>
            <div className="text-2xl font-bold text-blue-600">{stats.total_conversations}</div>
          </div>
          <div className="border border-tsushin-border rounded-lg p-4 bg-green-50 dark:bg-green-900/20">
            <div className="text-sm text-tsushin-slate mb-1">Total Messages</div>
          ...
          <div className="border border-tsushin-border rounded-lg p-4 bg-purple-50 dark:bg-purple-900/20">
            <div className="text-sm text-tsushin-slate mb-1">Vector Embeddings</div>
            <div className="text-2xl font-bold text-purple-600">{stats.total_embeddings}</div>
          </div>
          <div className="border border-tsushin-border rounded-lg p-4 bg-orange-50 dark:bg-orange-900/20">
            <div className="text-sm text-tsushin-slate mb-1">Storage Size</div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">
              ...
              <span className="text-tsushin-muted ml-2 font-normal">
          ...
          <div className="space-y-4 pt-4 border-t border-tsushin-border">
            <div>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
              ...
              <p className="text-xs text-tsushin-muted mt-1 ml-6">
              ...
              <>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs text-tsushin-muted">
      ...
      {/* Bulk Actions */}
      <div className="border border-tsushin-border rounded-lg p-4 bg-tsushin-ink">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="px-3 py-2 border border-tsushin-border rounded-md w-24 bg-tsushin-surface text-white"
              min="1"
            />
            <span className="py-2 text-sm text-tsushin-slate">days old</span>
            <button
          ...
          {cleanPreview && (
            <div className="mt-2 p-2 bg-orange-50 dark:bg-orange-900/20 border border-tsushin-border border-orange-200 rounded text-xs">
              <p className="font-medium">Preview: {cleanPreview.deleted_count} messages will be deleted</p>
              <ul className="list-disc list-inside mt-1 text-tsushin-slate">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-tsushin-ink border-b">
              <tr>
                <th className="px-4 py-3 text-left text-sm font-medium text-tsushin-muted">Sender</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-tsushin-muted">Messages</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-tsushin-muted">Last Activity</th>
                <th className="px-4 py-3 text-right text-sm font-medium text-tsushin-muted">Actions</th>
              </tr>
            </thead>
            <tbody>
              {conversations.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-tsushin-muted">
                ...
                conversations.map((conv) => (
                  <tr
                    key={conv.sender_key}
                    className={`border-b hover:bg-tsushin-surface bg-tsushin-ink ${selectedSenderKey === conv.sender_key ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}
                  >
                    <td className="px-4 py-3">
                      <div className="font-medium">{conv.sender_name || conv.sender_key}</div>
                      {conv.sender_name && (
                        <div className="text-xs text-tsushin-muted">{conv.sender_key}</div>
                      )}
                    </td>
            ...
            <div className="overflow-y-auto p-6 space-y-6 flex-1">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedConversation.working_memory.map((msg, i) => (
                    <div key={i} className="border border-tsushin-border rounded p-3 bg-tsushin-ink">
                      <div className="flex justify-between mb-1">
                        <span className={`text-sm font-medium ${msg.role === 'user' ? 'text-blue-600' : 'text-green-600'}`}>
                          {msg.role.toUpperCase()}
                        </span>
                        <span className="text-xs text-tsushin-muted">{formatDateTimeFull(msg.timestamp)}</span>
                      </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedConversation.episodic_memory.length === 0 ? (
                    <p className="text-sm text-tsushin-muted">No episodic memories found</p>
                  ) : (
                    selectedConversation.episodic_memory.map((mem, i) => (
                      <div key={i} className="border border-tsushin-border rounded p-3 bg-purple-50 dark:bg-purple-900/20">
                        <div className="flex justify-between mb-1">
                          <span className="text-xs text-purple-600 font-medium">
                            Similarity: {(mem.similarity * 100).toFixed(1)}%
                          </span>
                          <span className="text-xs text-tsushin-muted">{formatDateTimeFull(mem.timestamp)}</span>
                        </div>
              ...
              <div>
                ...
                {Object.keys(selectedConversation.semantic_facts).length === 0 ? (
                  <p className="text-sm text-tsushin-muted">No learned facts yet</p>
                ) : (
                  <div className="grid grid-cols-2 gap-4">
                    {Object.entries(selectedConversation.semantic_facts).map(([topic, facts]: [string, any]) => (
                      <div key={topic} className="border border-tsushin-border rounded p-3 bg-green-50 dark:bg-green-900/20">
                        ...
                        <ul className="space-y-1">
                          {Object.entries(facts).map(([key, value]: [string, any]) => (
                            <li key={key} className="text-sm">
                              ...
                              {value.confidence && (
                                <span className="text-xs text-tsushin-muted ml-1">
