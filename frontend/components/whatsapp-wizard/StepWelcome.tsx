...
export default function StepWelcome() {
  const { state, nextStep } = useWhatsAppWizard()
  const isTester = state.instanceType === 'tester'
  const setupItems = isTester
    ? [
        {
          icon: '1',
          title: 'Create tester bridge',
          desc: 'Start a dedicated WhatsApp MCP tester instance.',
        },
        {
          icon: '2',
          title: 'Link tester phone',
          desc: 'Scan a QR code with the phone used for QA runs.',
        },
        {
          icon: '3',
          title: 'Validate bot paths',
          desc: 'Use the tester instance for tester-to-bot WhatsApp checks.',
        },
      ]
    : [
        {
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-xl font-bold text-white mb-2">
          {isTester ? 'Connect WhatsApp Tester' : 'Connect WhatsApp'}
        </h3>
        <p className="text-tsushin-slate max-w-md mx-auto">
          {isTester
            ? 'This wizard will guide you through creating a QA tester instance for end-to-end WhatsApp validation.'
