...
export default function StepGroupConfig() {
  ...
  const [monitorAll, setMonitorAll] = useState((state.createdInstance?.group_filters ?? []).length === 0)
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const filters = monitorAll ? [] : groupFilters
      await api.updateMCPInstanceFilters(instanceId, {
        group_filters: filters,
        group_keywords: groupKeywords,
      })
      setFiltersData({ group_filters: filters, group_keywords: groupKeywords })
      markStepComplete(5)
      nextStep()
    } catch (e: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h4 className="text-sm font-semibold text-white">Monitor All Groups</h4>
          <InfoTooltip
            text="When ON, the agent listens to every group it's a member of. When OFF, you can select specific groups to monitor."
            position="right"
          />
        </div>
        <ToggleSwitch checked={monitorAll} onChange={(val) => { setMonitorAll(val); if (val) setGroupFilters([]) }} size="md" />
      </div>

      {/* Group Filters (shown when monitorAll is OFF) */}
      {!monitorAll && (
        <div>
          <label className="block text-sm font-medium text-white mb-1">
            Group Allowlist{' '}
            <InfoTooltip
              text="Add specific groups to monitor. Only these groups will trigger AI responses."
              position="right"
            />
          </label>
          <p className="text-xs text-tsushin-slate mb-2">
            Select the specific groups your agent should listen to.
          </p>
          <TypeaheadChipInput
            value={groupFilters}
            ...
            emptyStateText="No groups configured."
      ...
      {showAdvanced && (
        <div>
          <label className="block text-sm font-medium text-white mb-1">
            ...
            <InfoTooltip
              text="Words that make the agent respond in a group, besides @mentions. For example: 'help', 'support', 'bot'."
              position="right"
            />
          </label>
          <p className="text-xs text-tsushin-slate mb-2">
            Words that make the agent respond in a group, besides @mentions.
          </p>
