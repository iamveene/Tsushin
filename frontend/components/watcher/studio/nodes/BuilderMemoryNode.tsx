'use client'
import { memo } from 'react'
import { Handle, Position, type NodeProps } from '@xyflow/react'
import type { BuilderMemoryData } from '../types'

const ISOLATION_LABELS: Record<string, string> = { isolated: 'Isolated', shared: 'Shared', channel_isolated: 'Per-Channel' }

function BuilderMemoryNode({ data, selected }: NodeProps) {
  const d = data as BuilderMemoryData
  return (
    <div role="group" aria-label="Memory Configuration"
      className={`builder-node builder-node-memory px-4 py-3 rounded-xl border transition-all duration-200 ${selected ? 'border-blue-400 shadow-glow-sm' : 'border-tsushin-border hover:border-blue-400/50'} bg-tsushin-surface`}>
      <Handle type="source" position={Position.Right} className="!bg-blue-400 !border-tsushin-surface !w-3 !h-3" />
      <Handle type="target" position={Position.Left} className="!bg-blue-400 !border-tsushin-surface !w-3 !h-3" />
      <div className="flex items-center gap-2.5">
        <div className="w-8 h-8 rounded-lg bg-blue-500/20 flex items-center justify-center flex-shrink-0">
          <svg className="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" /></svg>
        </div>
        <div className="min-w-0">
          <p className="text-white text-sm font-medium">Memory</p>
          <div className="flex items-center gap-2 text-2xs">
            <span className="text-blue-300/70">{ISOLATION_LABELS[d.isolationMode] || d.isolationMode}</span>
            <span className="text-tsushin-muted">{d.memorySize} msgs</span>
          </div>
          {d.enableSemanticSearch && <span className="text-2xs text-blue-300/50">Semantic Search</span>}
        </div>
      </div>
    </div>
  )
}
export default memo(BuilderMemoryNode)
