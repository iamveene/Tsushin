...
function BuilderSkillNode({ data, selected }: NodeProps) {
  ...
  return (
    <div role="group" aria-label={`Skill: ${d.skillName}`}
      ...
      <Handle type="source" position={Position.Right} className="!bg-teal-400 !border-tsushin-surface !w-3 !h-3" />
      <Handle type="target" position={Position.Left} className="!bg-teal-400 !border-tsushin-surface !w-3 !h-3" />
