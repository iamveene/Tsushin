...
function BuilderKnowledgeNode({ data, selected }: NodeProps) {
  ...
  return (
    <div role="group" aria-label={`Knowledge: ${d.filename}`}
      ...
      <Handle type="source" position={Position.Right} className="!bg-violet-400 !border-tsushin-surface !w-3 !h-3" />
      <Handle type="target" position={Position.Left} className="!bg-violet-400 !border-tsushin-surface !w-3 !h-3" />
