...
def _create_test_agent(db_session, tenant_id="test-tenant"):
    ...
    contact = Contact(
        friendly_name=f"sentinel-edge-agent-{_test_agent_counter}",
        role="agent",
        tenant_id=tenant_id,
    )
    ...
    agent = Agent(
        ...
        system_prompt="Test agent for sentinel edge case tests",
    )
...
class TestProfileEdgeCases:
    ...
    async def test_multiple_agents_different_profiles(
        ...
        agent_a = _create_test_agent(db_session, tenant_id)
        agent_b = _create_test_agent(db_session, tenant_id)
