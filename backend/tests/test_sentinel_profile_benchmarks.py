...
Uses PostgreSQL with transactional rollback and mocked LLM calls.
...
def _create_test_agent(db_session, tenant_id="test-tenant"):
    ...
    contact = Contact(
        friendly_name=f"sentinel-bench-agent-{_test_agent_counter}",
        role="agent",
        tenant_id=tenant_id,
    )
    ...
    agent = Agent(
        ...
        system_prompt="Test agent for sentinel benchmark tests",
    )
