...
""").strip()
    ...
    async def _solve_image_captcha(self, provider, params: Dict[str, Any]) -> BrowserResult:
        ...
        solver_provider = str(
            params.get("solver_provider") or params.get("captcha_solver_provider") or "ollama"
        ).strip().lower()
        try:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                for candidate_path in self._captcha_image_variants(image_path, params):
                    if solver_provider == "gemini":
                        guesses = await self._captcha_guesses_from_gemini(candidate_path, params)
                    else:
                ...
                if state_data.get("successVisible") or not state_data.get("captchaVisible"):
                    return BrowserResult(
                        ...
                        data={
                            ...
                            "model": self._captcha_model_name(params, solver_provider),
...
Return JSON array only:"""
    ...
    async def _execute_action(self, provider, action_def: Dict) -> BrowserResult:
        ...
        # 35b: Rich action set
            ...
            if solver_provider in {"ollama", "gemini"}:
